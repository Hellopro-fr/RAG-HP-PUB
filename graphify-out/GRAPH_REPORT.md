# Graph Report - scoped --update (semantic refresh of 9 in-scope docs)  (2026-05-22)
...
## Summary
- 4828 nodes · 9778 edges · 144 communities detected
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 3338 edges (avg confidence: 0.61)
...
1. `CrawlerManager` - 264 edges
2. `DomainFR` - 146 edges
3. `IncludeInArchive` - 131 edges
4. `ReindexResponse` - 131 edges
5. `CrawlStatus` - 130 edges
6. `DetectionMode` - 114 edges
7. `LanguageDetector` - 111 edges
8. `CrawlerManager` - 81 edges
9. `Configuration` - 67 edges
10. `ScrapeResult` - 57 edges
...
- **** —  [EXTRACTED 1.00]
- **** —  [EXTRACTED 1.00]
- **** — crawler_stash_unstash_gcs, tools_upload_daemon, tools_download_daemon [EXTRACTED 1.00]
- **** — crawler_stash_unstash_followup_fixes, crawler_stash_unstash_gcs, crawler_crawler_manager [EXTRACTED 1.00]
- **** — mcp_gateway_authorize_consent_port, mcp_gateway_oauth2_authorize, mcp_gateway_sso_client [EXTRACTED 1.00]
- **stash/unstash 2-phase commit pipeline** —  [EXTRACTED 1.00]
- **stash bind-mount + cleanup design package** —  [EXTRACTED 1.00]
- **Redis loss/progress stall failure pipeline** — redis_health_monitor_ts, progress_monitor_ts, main_ts, crawler_manager_py, concept_failure_cause, fonctions_scrapping_php [EXTRACTED 0.95]
- **Stash/Archive cross-replica serialization pattern** — concept_lock_heartbeat, concept_ownership_safe_lock, concept_stash_lock, concept_archive_lock, concept_proxy_next_upstream_off, crawler_manager_py [EXTRACTED 0.95]
- **Camoufox OOM kill+reap recovery** — concept_browser_kill_pattern, concept_kill_browser_helper, concept_reap_delay, main_ts, concept_oom_relaunch_loop [EXTRACTED 0.95]

## Communities

### Community 0 - "Milvus CRUD Layer"
Cohesion: 0.01
Nodes (392): delete_key(), get_json(), Deletes a key from Redis., Sets a dictionary for a key, serializing it to JSON., Gets a dictionary for a key, deserializing it from JSON., Deletes a key from Redis., Atomically decrements a key's value by 1, with a floor of 0 (never goes negative, Sets a dictionary for a key, serializing it to JSON. (+384 more)

### Community 1 - "Node.js Crawler Core"
Cohesion: 0.02
Nodes (268): _cache_key(), check_url(), _compare_without_scheme(), DomainCache, DomainFR, _french_region_priority(), get_domain_from_url(), _identify_decision_case() (+260 more)

### Community 2 - "CrawlerManager Python + DLQ"
Cohesion: 0.02
Nodes (333): BaseModel, archive_crawl_to_gcs(), ArchiveResponse, CapacityResponse, clear_pending_callbacks(), CrawlMode, CrawlRequest, CrawlResponse (+325 more)

### Community 3 - "Rust Service Clients"
Cohesion: 0.02
Nodes (75): DLQArchiver, main(), Archives a batch of messages and ACKs/NACKs them individually., Main resilient loop to consume messages., Establishes and re-establishes connections to RabbitMQ and Elasticsearch., Declares queues and sets up consumers., Callback to buffer incoming messages., Recursively finds 'embedding' keys, extracts them into a dictionary with their J (+67 more)

### Community 4 - "Milvus Concurrency Guard"
Cohesion: 0.02
Nodes (113): killBrowserProcesses(), Per-replica Docker PID-namespace isolation guarantee, BROWSER_KILL_PATTERN constant, Infinite OOM_RELAUNCH loop on shop.monlabofermier.fr (crawl 6334), killBrowserProcesses() helper, Non-goals: page-cache subtraction, drop_caches, watchdog tuning, Operator smoke verification plan, Per-token rationale table (chrome/chromium/firefox/camoufox/playwright/headless_shell) (+105 more)

### Community 5 - "Redis Cache Rationale"
Cohesion: 0.01
Nodes (193): Read isError from _callback_payload.json in the crawl storage dir.     Returns, Read isError from _callback_payload.json in the crawl storage dir.     Returns, Raise 503 BIND_MOUNT_MISSING if path is not a real mount point.          Detec, Atomic compare-and-delete via Lua script. Returns True if the lock was, Stash a terminal crawl's storage dir to GCS (under gs://{bucket}/stash/) to free, Acquire a Redis lock with TTL, value = REPLICA_ID. Returns the value on, Atomic compare-and-delete via Lua script. Returns True if the lock was, Stash a terminal crawl's storage dir to GCS (under gs://{bucket}/stash/) to free (+185 more)

### Community 6 - "Pydantic Request Schemas"
Cohesion: 0.02
Nodes (204): Go vs Express benchmark — crawler-monitor-backend (2026-04), queue.Analyze CPU + JSON parsing benchmark (Go), RAM idle / under-load benchmark (distroless static), WebSocket broadcast p99 benchmark (gorilla/ws hub), Final review 6 blockers (commits 21bf809f..7ebbb0c8), cache_or_execute(), _client_name(), close_redis_pool() (+196 more)
...
Cohesion: 0.01
Nodes (152): Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica., Gracefully shut down all locally running crawlers on this replica. (+144 more)
...
Cohesion: 0.03
Nodes (91): _build_challenge_error_msg(), check_url_only(), detect_french(), detect_french_batch(), detect_french_debug(), _detect_single_url(), health_check(), _homepage_of() (+83 more)

### Community 13 - "Update-Mode Queue Builder"
Cohesion: 0.03
Nodes (114): 'admission_rejected' DetectionResponse method, AlternativeUrl schema (url, method, reliability, validated, region_priority?), api-gateway per-service downstream timeout map (detection=180s), API hreflang/data-lang same-host validation gate, DomainFR._is_valid_language_alternative static helper (Python), Apify proxy (APIFY_PROXY), call_api_hellopro additive extension (&$responseHeaders + ?$connectTimeout), BO Marketplace api-detection-langue-fr Caller Contract — Plan (+106 more)
...
Cohesion: 0.05
Nodes (97): ChatBaseURL, ChatProvider, CollectionName, CollectionNameGraph, Enum for the possible collection names.     The values correspond to the string, Enum for the possible collection names.     The values correspond to the string, # TODO:, Enum (+89 more)

### Community 17 - "Regional Path Exclusion Rationale"
Cohesion: 0.02
Nodes (78): _LockHeartbeat, Returns (archive_path, is_temporary).         is_temporary=True means the file, Returns (archive_path, is_temporary).         is_temporary=True means the file, Triggers a GCS download via the host-side download daemon and waits for the resu, Returns (archive_path, is_temporary).         is_temporary=True means the file, Triggers a GCS download via the host-side download daemon and waits for the resu, Triggers a GCS download via the host-side download daemon and waits for the resu, Returns (archive_path, is_temporary).         is_temporary=True means the file (+70 more)

### Community 18 - "Qdrant CRUD Layer"
Cohesion: 0.03
Nodes (64): _classify_exit_code(), _parse_iso_naive_utc(), Force a job to a terminal status (finished/failed).         Used to clean up st, Kill a process and all its children via the process group., Force a job to a terminal status (finished/failed).         Used to clean up st, Maps internal error codes to human-readable French messages for DB storage., Kill a process and all its children via the process group., Publie une mise à jour du statut d'un job sur le canal Pub/Sub de Redis. (+56 more)
...
Cohesion: 0.05
Nodes (29): Exit code 5 — Redis connection lost (failure_cause=redis_lost), Exit code 6 — Progress stall (failure_cause=progress_stalled), failure_cause cross-language contract vocabulary, PHP handle_crawler_webhook (fonctions_scrapping.php) — failure_cause consumer, PROGRESS_STALL_THRESHOLD_MS env var (default 600000), REDIS_LOSS_THRESHOLD_MS env var (default 60000), DedupManager, failure_cause=progress_stalled (exit code 6) (+21 more)

### Community 21 - "DeepSeek OCR Extractor"
Cohesion: 0.05
Nodes (50): api-gateway-go nginx.conf, api-gateway nginx.conf, BaseSettings, Camoufox OOM Restart Loop Plan, Camoufox OOM Restart Loop Design Spec, archive_lock:{id} Redis lock, BROWSER_KILL_PATTERN constant, camoufox-js (stealth Firefox) (+42 more)
...
Cohesion: 0.06
Nodes (50): POST /archive/{crawl_id}, cleanup_archives background task, crawler_archives/ -> /app/archives, crawler_manager.py, crawler-service CLAUDE.md, crawler_stash/ (host) -> /app/stash, crawler_stash_download_requests/ -> /app/gcs-stash-requests, crawler_stash_download_results/ -> /app/gcs-stash-downloads (+42 more)
...
Cohesion: 0.07
Nodes (33): Incident crawl 6250 on 2026-05-20 (lock heartbeat motivator), Lock heartbeat (stash + archive) doc section, Lock heartbeat tunable settings (LOCK_HEARTBEAT_INTERVAL_SECONDS, LOCK_HEARTBEAT_MAX_DURATION_SECONDS, STASH/ARCHIVE/UNSTASH_LOCK_TTL_SECONDS), nginx proxy_next_upstream off on /crawler/(stash|unstash|archive)/, Spec 2026-05-21 stash-archive-lock-heartbeat-design, _LockHeartbeat, Async context manager that renews a Redis lock TTL while a long-running     ope, Async context manager that renews a Redis lock TTL while a long-running     ope (+25 more)
...
Cohesion: 0.09
Nodes (2): DedupManager, RedisHealthMonitor
...
Cohesion: 0.33
Nodes (5): Returns all failed webhook callbacks stored in Redis., Returns all failed webhook callbacks stored in Redis., Returns all failed webhook callbacks stored in Redis., Returns all failed webhook callbacks stored in Redis., Returns all failed webhook callbacks stored in Redis.

### Community 57 - "Community 57"
Cohesion: 0.33
Nodes (5): Clears all failed webhook callbacks from Redis. Returns number of keys deleted., Clears all failed webhook callbacks from Redis. Returns number of keys deleted., Clears all failed webhook callbacks from Redis. Returns number of keys deleted., Clears all failed webhook callbacks from Redis. Returns number of keys deleted., Clears all failed webhook callbacks from Redis. Returns number of keys deleted.
...
## Knowledge Gaps
- **1454 isolated node(s):** `Enum for the possible collection names.     The values correspond to the string`, `Enum for the possible collection names.     The values correspond to the string`, `DLQProperties`, `Creates a dictionary of headers for a DLQ message, compatible with both pika and`, `Creates pika.BasicProperties for a DLQ message. For backward compatibility with` (+1449 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 31`** (31 nodes): `DedupManager`, `.addUrl()`, `.cleanup()`, `.connect()`, `.constructor()`, `.disconnect()`, `.ensureTtl()`, `.filterNewBlockedBatch()`, `.getAllUrls()`, `.getAllUrlsIterator()`, `.getCount()`, `.isKnown()`, `.isKnownBatch()`, `.loadFromIterator()`, `.loadFromList()`, `RedisHealthMonitor`, `.attach()`, `.constructor()`, `.evaluate()`, `.fire()`, `.onError()`, `.onSuccess()`, `.snapshot()`, `.start()`, `.stop()`, `clock()`, `onLost()`, `DedupManager.monitor.test.ts`, `DedupManager.ts`, `RedisHealthMonitor.test.ts`, `RedisHealthMonitor.ts`
...
- **Thin community `Community 46`** (9 nodes): `test_lua_scripts.py`, `Tests for Lua script string definitions. Validates that the scripts are well-fo`, `TestLuaScripts`, `.test_acquire_script_contains_expected_commands()`, `.test_acquire_script_is_non_empty_string()`, `.test_correct_counters_script_contains_expected_commands()`, `.test_correct_counters_script_is_non_empty_string()`, `.test_release_script_contains_expected_commands()`, `.test_release_script_is_non_empty_string()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (9 nodes): `QdrantWebsiteCrud.py`, `ModelConfig`, `QdrantWebsiteCrud`, `.delete_website()`, `._get_or_create_collection()`, `.get_website()`, `.__init__()`, `.insert_website()`, `.update_website()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (6 nodes): `AnonymizeText`, `.anonymize_text()`, `.normalize_text()`, `.presidio_anonymizer()`, `gen_email_uuid()`, `AnonymizeText.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 58`** (5 nodes): `rabbitmq_connection.py`, `RabbitMQConnection`, `.create_connection()`, `.__init__()`, `Crée une connexion RabbitMQ avec un nombre limité de tentatives.          :param`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 66`** (3 nodes): `logging_config.py`, `Configure root logger with a stdout handler.      Safe to call multiple times`, `setup_logging()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 67`** (3 nodes): `conftest.py`, `Pytest conftest for tools/ tests.  Adds the tools/ directory to sys.path so te`, `conftest.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 68`** (3 nodes): `export_embedding_model()`, `Charge le modèle d'embedding, exporte le module Transformer sous-jacent en ONNX,`, `export_embedding_to_onnx.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 69`** (3 nodes): `export_reranker_model()`, `Charge le modèle de reranking, l'exporte au format ONNX,     et génère le fichie`, `export_reranker_to_onnx.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (3 nodes): `auth.py`, `Verifies the API key if API_KEY is configured in settings.     If API_KEY is not`, `verify_api_key()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 76`** (2 nodes): `lua_scripts.py`, `Atomic Lua scripts for Redis-based concurrency guard. All slot operations (acqu`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 77`** (2 nodes): `milvus_lock.py`, `Shared threading lock for pymilvus connection management.  All Milvus CRUD and I`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 78`** (2 nodes): `Claude Config Optimization Plan`, `20-25% Token Consumption Reduction`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 79`** (2 nodes): `DLQ Manager UX Improvements (2026-04-11)`, `dlq-manager-service`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (2 nodes): `Crawler Monitor Dataset & Queue Insights (2026-04-12)`, `crawler-monitor-backend / frontend`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 81`** (2 nodes): `Rationale: Reduce token consumption by 20-25% per conversation`, `Claude Config Optimization (2026-04-16)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 83`** (2 nodes): `metrics.py`, `Prometheus metrics for api-detection-langue-fr.  Exposed at /metrics. Used to`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 84`** (2 nodes): `metrics.py`, `Prometheus metrics for api-detection-langue-fr.  Exposed at /metrics. Used to`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 85`** (2 nodes): `Rationale: TTL/interval=6 gives 5 missed-renewal slack vs Redis latency`, `Settings: STASH_LOCK_TTL=1800, ARCHIVE_LOCK_TTL=1800, HEARTBEAT_INTERVAL=300, HEARTBEAT_MAX_DURATION=14400`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `Creates a dictionary of headers for a DLQ message, compatible with both pika and`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (1 nodes): `Creates pika.BasicProperties for a DLQ message. For backward compatibility with`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (1 nodes): `Creates a dictionary of headers for a DLQ message based on an aio_pika message.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 91`** (1 nodes): `Creates a persistent aio_pika.Message ready for the Dead Letter Queue.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 92`** (1 nodes): `Cleans up whitespace and removes control characters.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 96`** (1 nodes): `Nettoie une chaîne de texte en normalisant les espaces et en corrigeant`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 107`** (1 nodes): `ignoreHTTPSErrors in Camoufox/Chromium`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 115`** (1 nodes): `A stale 'running' job marked failed must decrement the global counter.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 116`** (1 nodes): `If somehow we hit stale handler with terminal status, skip decrement.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (1 nodes): `When marker says finished, decrement + lock release + set_json with finished, NO`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 118`** (1 nodes): `When marker says failed, same reconcile path but Redis status=failed. Webhook st`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (1 nodes): `Marker None → existing stale-failure path runs (webhook sent, status=failed).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (1 nodes): `Marker invalid (helper returned None for unknown final_status) → fall through to`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (1 nodes): `Current admitted in-flight count (unsynchronized read for observability).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 125`** (1 nodes): `Number of coalesced calls served from a shared future.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (1 nodes): `Normalise l'encodage du contenu HTML en UTF-8.          Reproduit le comportem`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `Test endpoint /health`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (1 nodes): `Test endpoint /check-url avec TLD .fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (1 nodes): `Test endpoint /check-url sans indicateur FR`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (1 nodes): `Test endpoint /detect avec contenu HTML fourni`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `Test validation endpoint /detect-batch avec liste vide`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `Test endpoint racine /`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `Mode simple ne doit pas détecter les liens hreflang`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `Mode complete doit détecter les liens hreflang`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `Verify CAMOUFOX_ENABLED setting is respected.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `On happy path, page.unroute_all is called before context.close.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `A mid-fetch exception still triggers context.close and browser.close (try/finall`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 143`** (1 nodes): `Extrait le nom de domaine principal d'une URL.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Résout une URL relative en URL absolue.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `Détermine si l'URL a un signal très fort de site français.                  Le`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (1 nodes): `Vérifie si une URL indique explicitement une version française.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (1 nodes): `Return True only if candidate_url is plausibly a language variant of homepage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (1 nodes): `Compare two URLs ignoring scheme (http vs https) and trailing slashes.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `Determine la priorite regionale d'une URL francaise.          Retourne:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (1 nodes): `Identifie le cas de decision applique pour le debug.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `Fallback via API Pemavor pour la résolution de redirections.         À implémen`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `Forward compat: an old entry lacking 'requested_url' should still be readable.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (1 nodes): `When Phase 1 hits a variant-eligible error, Phase 2 retries variants.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 154`** (1 nodes): `Pass 2 retries fetch_failed + challenge_page only — not http_error/soft_404.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Maps internal error codes to hum`** (1 nodes): `When the homepage fallback hits a Cloudflare/DataDome challenge page,         r`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Maps internal error codes to hum`** (1 nodes): `Verify CAMOUFOX_ENABLED setting is respected.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Exit code 3 OOM_RELAUNCH contrac`** (1 nodes): `On happy path, page.unroute_all is called before context.close.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `.error marker`** (1 nodes): `A mid-fetch exception still triggers context.close and browser.close (try/finall`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Maps internal error codes to human-r`** (1 nodes): `When Playwright returns no Response, status_code defaults to 0.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Exit code 3 OOM_RELAUNCH contract (u`** (1 nodes): `Number of coalesced calls served from a shared future.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Returns (error_message, failure_caus`** (1 nodes): `Maps internal error codes to human-readable French messages for DB storage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 164`** (1 nodes): `Maps internal error codes to human-readable French messages for DB storage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 165`** (1 nodes): `Exit code 3 OOM_RELAUNCH contract (unchanged)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 166`** (1 nodes): `Maps internal error codes to human-readable French messages for DB storage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 167`** (1 nodes): `Returns (error_message, failure_cause) for a subprocess exit code.          Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 168`** (1 nodes): `Maps internal error codes to human-readable French messages for DB storage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 169`** (1 nodes): `Returns (error_message, failure_cause) for a subprocess exit code.          Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 170`** (1 nodes): `exit_code=-1 → classifier says 'oom_max_restarts'.         Explicit failure_cau`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 171`** (1 nodes): `Without an explicit kwarg, classifier default 'oom_max_restarts' is used.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (1 nodes): `Every exit_code=-1 caller passes its own explicit cause correctly.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 173`** (1 nodes): `Maps internal error codes to human-readable French messages for DB storage.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (1 nodes): `Returns (error_message, failure_cause) for a subprocess exit code.          Re`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `cache_service.py (Redis lock helper)` connect `Pydantic Request Schemas` to `Milvus CRUD Layer`, `Update-Mode Queue Builder`?**
  _High betweenness centrality (0.133) - this node is a cross-community bridge._
- **Why does `main()` connect `Header Footer Extractor` to `LLM gRPC Client`?**
  _High betweenness centrality (0.105) - this node is a cross-community bridge._
- **Are the 212 inferred relationships involving `CrawlerManager` (e.g. with `CrawlStatus` and `IncludeInArchive`) actually correct?**
  _`CrawlerManager` has 212 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 126 inferred relationships involving `str` (e.g. with `create_dlq_headers()` and `._preprocess_html()`) actually correct?**
...
- **Are the 127 inferred relationships involving `ReindexResponse` (e.g. with `CrawlerManager` and `.reindex_storage()`) actually correct?**
  _`ReindexResponse` has 127 INFERRED edges - model-reasoned connections that need verification._