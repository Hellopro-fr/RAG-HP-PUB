# Graph Report - unified --update v7  (2026-05-19)
...
## Summary
- 3647 nodes · 7477 edges · 117 communities detected
- Extraction: 64% EXTRACTED · 36% INFERRED · 0% AMBIGUOUS · INFERRED: 2726 edges (avg confidence: 0.61)
...
1. `CrawlerManager` - 165 edges
2. `DomainFR` - 146 edges
3. `DetectionMode` - 114 edges
4. `LanguageDetector` - 111 edges
5. `IncludeInArchive` - 79 edges
6. `ReindexResponse` - 79 edges
7. `CrawlStatus` - 78 edges
8. `Configuration` - 67 edges
9. `ScrapeResult` - 57 edges
10. `CrawlerManager` - 56 edges
...
Cohesion: 0.02
Nodes (266): _map_error_to_message(), archive_crawl_to_gcs(), ArchiveResponse, CapacityResponse, clear_pending_callbacks(), CrawlMode, CrawlRequest, CrawlResponse (+258 more)

### Community 1 - "Node.js Crawler Core"
Cohesion: 0.02
Nodes (246): BaseModel, InsertDevisRequest, _cache_key(), check_url(), _compare_without_scheme(), DomainCache, DomainFR, _french_region_priority() (+238 more)

### Community 2 - "CrawlerManager Python + DLQ"
Cohesion: 0.01
Nodes (183): _count_files_in_dir(), CrawlerManager, Kill a process and all its children via the process group., Returns (archive_path, is_temporary).         is_temporary=True means the file, Returns (archive_path, is_temporary).         is_temporary=True means the file, Triggers a GCS download via the host-side download daemon and waits for the resu, Triggers a GCS download via the host-side download daemon and waits for the resu, Publie une mise à jour du statut d'un job sur le canal Pub/Sub de Redis. (+175 more)
...
Cohesion: 0.01
Nodes (250): 'admission_rejected' DetectionResponse method, AlternativeUrl schema (url, method, reliability, validated, region_priority?), api-gateway per-service downstream timeout map (detection=180s), API hreflang/data-lang same-host validation gate, DomainFR._is_valid_language_alternative static helper (Python), Apify proxy (APIFY_PROXY), Go vs Express benchmark — crawler-monitor-backend (2026-04), queue.Analyze CPU + JSON parsing benchmark (Go) (+242 more)
...
Cohesion: 0.03
Nodes (85): AdmissionController, AdmissionMiddleware, FastAPI admission-control middleware.  Scope after the crawler carve-out refac, Atomic in-flight counter with a hard max.      Not a semaphore: acquire() does, Try to acquire a slot. Returns True on success, False if saturated., Release a slot. Defensive: does not go below zero., BaseHTTPMiddleware, reconcile_jobs() (+77 more)
...
- **Why does `libs/common-utils` connect `Milvus Concurrency Guard` to `Enums and Collection Types`, `Rust Service Request Models`?**
  _High betweenness centrality (0.141) - this node is a cross-community bridge._
- **Why does `common_utils.grpc_clients (client wrappers)` connect `Enums and Collection Types` to `Milvus Concurrency Guard`?**
  _High betweenness centrality (0.141) - this node is a cross-community bridge._
...
- **Are the 97 inferred relationships involving `str` (e.g. with `create_dlq_headers()` and `._preprocess_html()`) actually correct?**
