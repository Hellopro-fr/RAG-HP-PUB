...
async def execute_cypher(
    query: str, parameters: Optional[Dict[str, Any]] = None, read_only: bool = False
...
async def execute_batch_cypher(
    statements: List[Dict[str, Any]], transactional: bool = True
    ...
    try:
        async with grpc.aio.insecure_channel(GRAPH_DATABASE_SERVICE_URL) as channel:
            ...
            request = graph_database_pb2.ExecuteBatchCypherRequest(
                statements=pb_statements, transactional=transactional
            )
...
async def get_graph_schema(
    include_properties: bool = True, include_indexes: bool = False
) -> GraphSchema:
    """
    ...
    try:
        async with grpc.aio.insecure_channel(GRAPH_DATABASE_SERVICE_URL) as channel:
            ...
            request = graph_database_pb2.GetGraphSchemaRequest(
                include_properties=include_properties, include_indexes=include_indexes
            )
...
async def setup_schema(
    apply_constraints: bool = True, apply_indexes: bool = True
    ...
    try:
        async with grpc.aio.insecure_channel(GRAPH_DATABASE_SERVICE_URL) as channel:
            ...
            request = graph_database_pb2.SetupSchemaRequest(
                apply_constraints=apply_constraints, apply_indexes=apply_indexes
            )
