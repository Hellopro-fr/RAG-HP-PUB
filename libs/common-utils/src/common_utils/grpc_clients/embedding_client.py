...
async def get_embeddings(texts: List[str], source_service: str | None = None) -> List[List[float]]:
    """
    ...
    try:
        async with grpc.aio.insecure_channel(EMBEDDING_SERVICE_URL) as channel:
            ...
            request = embedding_pb2.EmbeddingsRequest(texts=texts, source_service=source_service)
...
async def get_embedding(text: str, source_service: str | None = None) -> List[float]:
    """
    Obtient l'embedding pour un seul texte.
    Wrapper pour la fonction de batching.
    """
    results = await get_embeddings([text], source_service=source_service)
