...
class QdrantProduitsCrud:
    ...
    def insert_produits(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[{model_key}][produits] Erreur Qdrant lors de l'insertion : {e}", exc_info=True)

    def update_produits(self, produit: Dict[str, Any]) -> Dict[str, Any]:
        data = produit
        model_config = ModelConfig()
        model_key = model_config.model_id
        ...
        try:
            ...
            self.logger.info(f"[{model_key}][produits] Mise à jour de {len(data)} entités dans '{self.collection}'...")
            ...
            self.logger.error(f"[{model_key}][produits] Erreur Qdrant lors de la mise à jour : {e}", exc_info=True)

    def delete_produits(self, produit: Dict[str, Any]) -> Dict[str, Any]:
        model_config = ModelConfig()
        model_key = model_config.model_id
        id_entity = produit.get("id")
        ...
        try:
            ...
            self.logger.info(f"[{model_key}] ✓ produit avec ID {id_entity} supprimé.")

            return {"status": "success", "message": f"produit {id_entity} supprimé."}
        except Exception as e:
    ...
    def get_produit(self, id_produit: str) -> Dict[str, Any]:
        ...
        try:
            ...
            filter_query = Filter(
                must=[FieldCondition(key="id_produit", match=MatchValue(value=id_produit))]
            )
