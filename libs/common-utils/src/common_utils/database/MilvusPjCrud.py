...
class MilvuspjCrud:
    ...
    def _get_or_create_collection(self, model_config: ModelConfig) -> Collection:
        ...
        if not utility.has_collection(collection_name):
            ...
            schema = CollectionSchema(fields, description=f"Collection de chunks de pj pour {model_key}")
    ...
    async def insert_pj(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"[{model_key}][pj] Insertion de batch de {len(datas)} entités dans '{self.collection.name}'...")
            ...
            self.logger.error(f"[{model_key}][pj] Erreur Milvus lors de l'insertion : {e}")
            return {
                "status": "error",
                "message": f"Erreur Milvus lors de l'insertion : {e}"
            }
        except Exception as e:
            self.logger.error(f"[{model_key}][pj] insertion de batch : {e}", exc_info=True)
            return {
                "status": "error",
                "message": f"Erreur inattendue : {e}"
            }
    
    async def update_pj(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        model_config = ModelConfig()
        model_key = model_config.model_id
        ...
        try:
            ...
            self.logger.error(f"[{model_key}][pj] Erreur Milvus lors de mise à jour : {e}")
            self.logger.error(f"Data : {datas}")
        except Exception as e:
            self.logger.error(f"[{model_key}][pj] Mise à jour de batch : {e}", exc_info=True)
            self.logger.error(f"Data : {datas}")

    async def delete_pj(self,data: Dict[str, Any]) -> Dict[str, Any]:
        model_config = ModelConfig()
        model_key = model_config.model_id
        ...
        try:
            ...
            if not id_entity_milvus:
                self.logger.error(f"[{model_key}][pj] Suppression sans ID.")
                return {
                    "status": "error",
                    "message": "Clé primaire (ID) requise pour la suppression."
                }

            self.logger.info(f"[{model_key}][pj] Suppression de l'entité avec ID {id_entity_milvus} dans '{self.collection.name}'...")
            ...
            return {
                "status": "success",
                "message": f"pj avec ID {id_entity_milvus} supprimé."
            }

        except MilvusException as e:
            self.logger.error(f"[{model_key}][pj] Erreur Milvus lors de la suppression : {e}")
        except Exception as e:
    ...
    async def get_pj(self,fichier_source: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[{model_key}][pj] Erreur Milvus lors de la récupération : {e}")
            return {
                "status": "error",
            ...
            self.logger.error(f"[{model_key}][pj] Erreur de Récupèration de pj : {e}", exc_info=True)
            return {
                "status": "error",
                "message": f"Erreur inattendue : {e}",
                "code": 500
