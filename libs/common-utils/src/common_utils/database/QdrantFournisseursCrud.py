...
class QdrantFournisseursCrud:
    ...
    def insert_fournisseurs(self, fournisseur: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[{model_key}][fournisseurs] Erreur Qdrant lors de l'insertion : {e}", exc_info=True)

    def update_fournisseurs(self, fournisseur: Dict[str, Any]) -> Dict[str, Any]:
        data = fournisseur
        model_config = ModelConfig()
        model_key = model_config.model_id
        ...
        try:
            ...
            self.logger.info(f"[{model_key}][fournisseurs] Mise à jour de {len(data)} entités dans '{self.collection}'...")
            ...
            self.logger.error(f"[{model_key}][fournisseurs] Erreur Qdrant lors de la mise à jour : {e}", exc_info=True)

    def delete_fournisseurs(self, fournisseur: Dict[str, Any]) -> Dict[str, Any]:
        model_config = ModelConfig()
        model_key = model_config.model_id
        id_entity = fournisseur.get("id")
        ...
        try:
            ...
            self.logger.info(f"[{model_key}] ✓ fournisseur avec ID {id_entity} supprimé.")
