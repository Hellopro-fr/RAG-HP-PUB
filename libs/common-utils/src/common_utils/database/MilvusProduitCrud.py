...
class MilvusProduitsCrud:
    ...
    def update_produits(self, produits: List[Dict[str, Any]], id_produit: str, correspondance_produit=None) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            correspondance_produit: Instance de MilvusProduitInserer pour gérer la table de correspondance
        ...
        try:
            ...
            if not produits or self.collection is None:
                return {
                    "status": "error",
            ...
            if not id_produit:
                return {
                    "status": "error",
                    "message": "id_produit requis pour la mise à jour."
                }

            self.logger.info(f"[{model_key}][Produits] Mise à jour pour id_produit: {id_produit}")

            # 1. Récupérer la correspondance
            if correspondance_produit is None:
            ...
            # 3. Supprimer les anciennes données dans 'produits'
            ...
            if insert_result and insert_result.get("status") == "success":
                ...
                correspondance_produit.insert_correspondance_produit(data_bo_milvus)
