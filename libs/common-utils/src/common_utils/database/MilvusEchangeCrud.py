...
class MilvusEchangeCrud:
    ...
    def _get_or_create_collection(self, model_config: ModelConfig) -> Collection:
        ...
        if not utility.has_collection(collection_name, using=self._CONNECTION_ALIAS):
            ...
            fields = [
                ...
                FieldSchema(
                    name="id_produit", dtype=DataType.VARCHAR, max_length=65535
                ),
                ...
                FieldSchema(
                    name="id_acheteur", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(name="text", dtype=DataType.VARCHAR, max_length=65535),
                FieldSchema(
                    name="conversation_id", dtype=DataType.VARCHAR, max_length=65535
                ),
            ...
            collection.create_index(
                field_name="conversation_id", index_name="idx_conversation_id"
            )
    ...
    def insert_echange(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][Echange] Insertion de batch de {len(datas)} entités dans '{self.collection.name}'..."
            )
    ...
    def update_echange(
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][Echange] Mise à jour pour conversation_id: {conversation_id}"
            )

            # 1. Récupérer la correspondance
            if correspondance_echange is None:
                from common_utils.database.MilvusEchangeInserer import (
                    MilvusEchangeInserer,
                )

                correspondance_echange = MilvusEchangeInserer()

            correspondance_result = (
                correspondance_echange.get_correspondance_by_conversation_id(
                    conversation_id
                )
            )
            if correspondance_result["status"] == "error":
                self.logger.error(
                    f"[{model_key}][Echange] Erreur récupération correspondance: {correspondance_result['message']}"
                )
                return {
                    "status": "error",
            ...
            if not ids_str:
                self.logger.error(
                    f"[{model_key}][Echange] Aucun ID trouvé dans la correspondance"
                )
                return {
                    "status": "error",
            ...
            if delete_result["status"] == "error":
                self.logger.error(
                    f"[{model_key}][Echange] Erreur suppression: {delete_result['message']}"
                )
                return {
                    "status": "error",
            ...
            delete_corr_result = (
                correspondance_echange.delete_correspondance_by_conversation_id(
                    conversation_id
                )
            )
            if delete_corr_result["status"] == "error":
                self.logger.error(
                    f"[{model_key}][Echange] Erreur suppression correspondance: {delete_corr_result['message']}"
                )
                return {
                    "status": "error",
            ...
            self.logger.info(
                f"[{model_key}][Echange] Suppression réussie. Réinsertion en cours..."
            )
            ...
            if insert_result and insert_result.get("status") == "success":
                ...
                self.logger.info(
                    f"[{model_key}][Echange] ✓ Mise à jour terminée avec succès."
                )
    ...
    def delete_echange(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][Echange] Suppression de l'entité avec ID {id_entity_milvus} dans '{self.collection.name}'..."
            )
    ...
    def delete_echanges_by_ids(self, ids: List[int]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][Echange] Suppression de {len(ids)} entités avec IDs {ids} dans '{self.collection.name}'..."
            )
