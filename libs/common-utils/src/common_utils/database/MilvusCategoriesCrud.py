...
class MilvusCategoriesCrud:
    ...
    def _get_or_create_collection(self, model_config: ModelConfig) -> Collection:
        ...
        if not utility.has_collection(collection_name):
            ...
            fields = [
                ...
                FieldSchema(name="rubrique_parent", dtype=DataType.VARCHAR, max_length=65535),
                FieldSchema(name="id_rubrique_parent", dtype=DataType.VARCHAR, max_length=65535),
                FieldSchema(name="nom_secteur", dtype=DataType.VARCHAR, max_length=65535),
                FieldSchema(name="id_secteur", dtype=DataType.VARCHAR, max_length=65535),
    ...
    def insert_categories(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[{model_key}][categories] Erreur Milvus lors de l'insertion : {e}")
            self.logger.error(f"Data : {data}")
        except Exception as e:
            self.logger.error(f"[{model_key}][categories] insertion de batch : {e}", exc_info=True)
            self.logger.error(f"Data : {data}")
    ...
    def update_categories(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not data.get("id"):
                self.logger.error(f"[{model_key}][categories] Mise à jour sans ID.")
                return {
                    "status": "error",
                    "message": "Clé primaire (ID) requise pour la mise à jour."
                }

            self.logger.info(f"[{model_key}][categories] Mise à jour de batch de {len(data)} entités dans '{self.collection.name}'...")
            ...
            self.logger.error(f"[{model_key}][categories] Erreur Milvus lors de mise à jour : {e}")
            self.logger.error(f"Data : {data}")
        except Exception as e:
            self.logger.error(f"[{model_key}][categories] Mise à jour de batch : {e}", exc_info=True)
            self.logger.error(f"Data : {data}")
    ...
    def delete_categories(self,data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not id_entity_milvus:
                self.logger.error(f"[{model_key}][categories] Suppression sans ID.")
                return {
                    "status": "error",
                    "message": "Clé primaire (ID) requise pour la suppression."
                }

            self.logger.info(f"[{model_key}][categories] Suppression de l'entité avec ID {id_entity_milvus} dans '{self.collection.name}'...")
