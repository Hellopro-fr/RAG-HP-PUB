...
class MilvusCategoriesCrud:
    ...
    def _get_or_create_collection(self, model_config: ModelConfig) -> Collection:
        ...
        if not utility.has_collection(collection_name, using=self._CONNECTION_ALIAS):
            ...
            fields = [
                ...
                FieldSchema(
                    name="rubrique_parent", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
                    name="id_rubrique_parent", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
                    name="nom_secteur", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
                    name="id_secteur", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
                    name="type_categorie", dtype=DataType.VARCHAR, max_length=65535
                ),
                FieldSchema(
    ...
    def insert_categories(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][categories] Insertion de batch de {len(datas)} entités dans '{self.collection.name}'..."
            )
    ...
    def update_categories(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][categories] Mise à jour de batch de {len(data)} entités dans '{self.collection.name}'..."
            )
    ...
    def delete_categories(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"[{model_key}][categories] Suppression de l'entité avec ID {id_entity_milvus} dans '{self.collection.name}'..."
            )
