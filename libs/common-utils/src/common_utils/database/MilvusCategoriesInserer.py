...
class MilvusCategoriesInserer:
    ...
    def _get_or_create_collection(self, model_config: ModelConfig) -> Collection:
        ...
        if not utility.has_collection(collection_name):
            ...
            fields = [
                ...
                FieldSchema(name="id_categorie_milvus", dtype=DataType.VARCHAR, max_length=65535),
            ...
            schema = CollectionSchema(fields, description=f"Collection de correspondance Milvus - BO Catégories")
    ...
    def insert_correspondance_categories(self, datas: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[Correspondace Categories BO-Milvus] Erreur Milvus lors de l'insertion : {e}")
            self.logger.error(f"Data : {datas}")
        except Exception as e:
            self.logger.error(f"[Correspondace Categories BO-Milvus] insertion de batch : {e}", exc_info=True)
            self.logger.error(f"Data : {datas}")