...
- **MilvusDocumentCrud / MilvusPjCrud**: `_validate_varchar_lengths()` pre-validates all VARCHAR fields before insert/upsert, raising `ValueError` with field name, length, and preview. `MilvusException` is now wrapped in `RuntimeError` with operation context before re-raising, ensuring readable DLQ headers.
