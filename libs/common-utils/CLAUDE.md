...
- **cache_service**: `init_redis_pool()` passes an explicit `retry=Retry(ExponentialBackoff(cap=1.0, base=0.05), 3)` and `retry_on_error=[ConnectionError, TimeoutError]`. Redis reaps idle connections server-side (`CONFIG timeout 300`); the pool keeps handing the reaped sockets out and `health_check_interval` only turns the failure into a failed PING. With no retry passed, redis-py builds `Retry(NoBackoff(), 0)` and an empty `retry_on_error` (verified on 5.2.1), so the first command on a reaped socket raised `ConnectionError: Connection closed by server.` — which **every helper in this module swallows into its default return value** (`None`, `0`, `[]`, `False`), indistinguishable from a real answer by the caller. `retry_on_error` is not optional: without it `_disconnect_raise` re-raises before the retry loop retries. Affects every service calling `init_redis_pool`.
- **cache_service**: `set_json_nx()` takes an optional `ttl`. SET NX with EX is one round trip, so the expiry applies only to the key the call creates — an existing key keeps its own. Added for `crawler-service`'s disk-recovery path, which must index an orphan without ever overwriting a blob whose read merely errored.
...
- **AnonymizeText**: the Presidio `AnalyzerEngine`/`AnonymizerEngine` are now lazy class-level singletons (`_get_engines()`) reused across documents, instead of being re-instantiated per call. Each construction loads the spaCy `en_core_web_lg` model (~1GB resident); the per-document reload was churning memory and pushing document-echange-processor-service toward OOM. Single event-loop thread → no lock needed.
