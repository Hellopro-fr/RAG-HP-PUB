...
- **Do not remember which services you touched — ask.** `python scripts/graphify_plan_update.py` lists exactly which areas are owed a re-extraction and why (changed doc = needs the LLM; file absent from the graph = the post-commit hook cannot admit it). Exit 0 means nothing is owed. `/graphify-refresh` runs the whole thing: plan, then each scoped update, then one re-labelling pass. Prefer it whenever more than one area may have moved.
