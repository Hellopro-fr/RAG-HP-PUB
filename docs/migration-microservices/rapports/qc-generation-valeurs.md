# Rapport de bascule — `qc-generation-valeurs`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-qc-generation-valeurs-1` et `-2`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `qc-generation-valeurs`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 2 | **1** au départ (≈ la moitié), ajustable sur la profondeur de la file |
| File(s) consommée(s) | `qc_valeurs_queue` sur le broker prod `10.0.1.216` | **Les mêmes**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
...
| Image | — | `qc-generation-valeurs:tracking-2026-09-23` depuis le 23/09 06:10-06:29 UTC (avant : `shadow-2026-09-15`) |
...
- secret `qc-generation-valeurs-rabbitmq`, clé `rabbitmq-url` : broker dev → **prod** (seule clé repointée à la bascule) ;
...
```bash
kubectl -n apps-microservices logs deploy/qc-generation-valeurs --tail=200
kubectl -n apps-microservices logs deploy/qc-generation-valeurs -f
kubectl -n apps-microservices get pods -l app=qc-generation-valeurs
...
resource.labels.container_name="qc-generation-valeurs"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=qc-generation-valeurs | tail -30
kubectl -n apps-microservices exec -it deploy/qc-generation-valeurs -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep qc_valeurs
```
...
| 22/09 10:47:32 → 10:48:11 | Secret `qc-generation-valeurs-rabbitmq` repointé sur la prod, `rollout` |
...
1. `kubectl -n apps-microservices scale deploy/qc-generation-valeurs --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=qc-generation-valeurs --timeout=60s` — sans cette attente, les deux côtés consomment en même temps quelques secondes (21/09)
3. VM GPU : `docker start rag-hp-pub-qc-generation-valeurs-1 rag-hp-pub-qc-generation-valeurs-2`
...
| 2 | **Réplicas** : 1 contre 2 sur la VM. Surveiller la profondeur de `qc_valeurs_queue` ; passer à 2 si elle monte | DevSecOps |
