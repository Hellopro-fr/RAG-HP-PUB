# Rapport de bascule — `website-database-qdrant-service`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-website-database-qdrant-service-1` à `-4`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `website-database-qdrant-service`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 4 | **2** (palier 1 du 24/09), ajustable sur la profondeur de la file |
| File consommée | `insertion_siteweb_queue` sur le broker prod `10.0.1.216` | **La même**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
| Écriture | Milvus prod (`milvus-prod.hello.dev.private.com` → `10.0.1.51:19530`) : `siteweb_2` (4,9 M d'entités) | **La même base, les mêmes collections** |
...
**Ce qui a changé côté configuration** — secret multi-clés `website-database-qdrant-secrets`, modifié clé par clé (`kubectl patch`, jamais recréé ; copie d'avant bascule `website-database-qdrant-secrets-prel4`) :
...
**Il ne fait pas qu'insérer** : il **supprime** les anciens morceaux d'une page (par domaine + type de page, ou par URL) puis réinsère. Un rollback données ne peut donc pas se limiter à « supprimer ce qui a été ajouté » ; il passe par la restauration de la sauvegarde.

**Tout ce qui arrive dans `insertion_siteweb_queue` finit dans Milvus prod**, quelle que soit sa provenance : la file est alimentée par `embedding-service`, resté sur la VM jusqu'à L6.
...
```bash
kubectl -n apps-microservices logs deploy/website-database-qdrant-service --all-pods --prefix --tail=200
kubectl -n apps-microservices logs deploy/website-database-qdrant-service --all-pods --prefix -f
kubectl -n apps-microservices get pods -l app=website-database-qdrant-service
...
resource.labels.container_name="website-database-qdrant-service"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=website-database-qdrant-service | tail -30
kubectl -n apps-microservices exec -it deploy/website-database-qdrant-service -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep -w insertion_siteweb_queue
```
...
```bash
kubectl -n apps-microservices port-forward deploy/website-database-qdrant-service 8530:8530   # puis http://localhost:8530/metrics
```
...
1. `kubectl -n apps-microservices scale deploy/website-database-qdrant-service --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=website-database-qdrant-service --timeout=60s`
3. VM GPU : `docker start rag-hp-pub-website-database-qdrant-service-1 rag-hp-pub-website-database-qdrant-service-2 rag-hp-pub-website-database-qdrant-service-3 rag-hp-pub-website-database-qdrant-service-4`
...
| 2 | Réplicas : 2 contre 4 sur la VM. Surveiller `insertion_siteweb_queue` aux heures chargées ; passer à 3 ou 4 si elle accumule (mémoire du cluster à vérifier avant) | DevSecOps |
