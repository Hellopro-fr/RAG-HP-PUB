# Rapport de bascule — `qc-equivalence`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-qc-equivalence-1` et `-2`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `qc-equivalence`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 2 | **1** au départ (≈ la moitié), ajustable sur la profondeur de la file |
| File(s) consommée(s) | `qc_equivalence_queue` et `qc_equivalence_bo_queue` sur le broker prod `10.0.1.216` | **Les mêmes**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
...
| Image | — | `qc-equivalence:tracking-2026-09-23` depuis le 23/09 06:10-06:29 UTC (avant : `shadow-2026-09-15`) |
...
- secret `qc-equivalence-rabbitmq`, clé `rabbitmq-url` : broker dev → **prod** (seule clé repointée à la bascule) ;
...
**Deux files** : `qc_equivalence_queue` et `qc_equivalence_bo_queue` (circuit back-office). Les deux sont consommées par le même pod.
...
```bash
kubectl -n apps-microservices logs deploy/qc-equivalence --tail=200
kubectl -n apps-microservices logs deploy/qc-equivalence -f
kubectl -n apps-microservices get pods -l app=qc-equivalence
...
resource.labels.container_name="qc-equivalence"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=qc-equivalence | tail -30
kubectl -n apps-microservices exec -it deploy/qc-equivalence -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep qc_equivalence
```
...
| 22/09 10:47:32 → 10:48:11 | Secret `qc-equivalence-rabbitmq` repointé sur la prod, `rollout` |
...
1. `kubectl -n apps-microservices scale deploy/qc-equivalence --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=qc-equivalence --timeout=60s` — sans cette attente, les deux côtés consomment en même temps quelques secondes (21/09)
3. VM GPU : `docker start rag-hp-pub-qc-equivalence-1 rag-hp-pub-qc-equivalence-2`
4. vérifier `consumers=2` sur chacune des deux files
...
| 2 | **Réplicas** : 1 contre 2 sur la VM. Surveiller la profondeur de `qc_equivalence_queue` et `qc_equivalence_bo_queue` ; passer à 2 si elle monte | DevSecOps |
