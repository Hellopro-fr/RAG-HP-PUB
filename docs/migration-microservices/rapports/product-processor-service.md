# Rapport de bascule — `product-processor-service`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-product-processor-service-1` à `-4`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `product-processor-service`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 4 | **2** (palier 1 du 24/09), ajustable sur la profondeur de la file |
| File consommée | `product_processing_queue` sur le broker prod `10.0.1.216` | **La même**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
| Sortie | `processed_data_exchange` / `data.ready_for_embedding` (vers `embedding-service`, resté sur la VM jusqu'à L6) | **Identique** |
...
**Ce qui a changé côté configuration** : secret `product-processor-rabbitmq`, clés `rabbitmq-url`, valeurs dev → **prod** (Secret Manager `platform-rabbitmq-url`, réécrite en DNS interne). Chaque clé posée a été relue par empreinte ; empreintes identiques à celles des conteneurs VM.
...
**Seul consumer GKE du lot avec un wrapper de déploiement automatique** sur `prod` : un merge qui le touche reconstruit et redéploie son image (`sha-…`). Image au moment de la bascule : `product-processor-service:sha-29e94ca`. Dès la bascule, il a traité des produits en attente (≈ 2 900 lignes de log, 0 erreur).

**Il ne touche pas Milvus.** Il prépare la donnée et la publie sur `processed_data_exchange` / `data.ready_for_embedding` (vers `embedding-service`, resté sur la VM jusqu'à L6). La suite de la chaîne passe donc par la VM puis revient sur GKE (les écrivains du lot) : un message peut traverser GKE → VM → GKE, via le même broker.
...
```bash
kubectl -n apps-microservices logs deploy/product-processor-service --all-pods --prefix --tail=200
kubectl -n apps-microservices logs deploy/product-processor-service --all-pods --prefix -f
kubectl -n apps-microservices get pods -l app=product-processor-service
...
resource.labels.container_name="product-processor-service"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=product-processor-service | tail -30
kubectl -n apps-microservices exec -it deploy/product-processor-service -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep -w product_processing_queue
```
...
```bash
kubectl -n apps-microservices port-forward deploy/product-processor-service 8530:8530   # puis http://localhost:8530/metrics
```
...
1. `kubectl -n apps-microservices scale deploy/product-processor-service --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=product-processor-service --timeout=60s`
3. VM GPU : `docker start rag-hp-pub-product-processor-service-1 rag-hp-pub-product-processor-service-2 rag-hp-pub-product-processor-service-3 rag-hp-pub-product-processor-service-4`
...
| 2 | **Gel** : tout merge `prod` touchant ce service redéploie l'image automatiquement ; à éviter jusqu'à la décision du 25/09 | Dev · LEAD |
