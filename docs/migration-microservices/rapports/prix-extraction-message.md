# Rapport de bascule — `prix-extraction-message`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-prix-extraction-message-1`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `prix-extraction-message`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 1 | 1 |
| File consommée | `prix_message_processing_queue` sur le broker prod `10.0.1.216` | **La même**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
...
| Image | — | `prix-extraction-message:tracking-2026-09-23` |
...
- secret `prix-extraction-message-rabbitmq`, clé `rabbitmq-url` : broker dev → **prod** ; le service lit `HP_TOKEN` (empreinte `1d02693e`) et `GEMINI_API_KEY` (empreinte `36c842e3`) du secret partagé `platform-llm-hp-secrets`, identiques VM et GKE ;
...
| Traitement de bout en bout | ✅ 23/09, catégorie `2005786` : 14 succès sur 164 → **28 lignes insérées dans Milvus prod** (via `prix-caracterisation` puis `prix-milvus-processor`), 0 doublon |
...
**Pas de fenêtre tarifaire** (Gemini). Hors incident, `consumers=1` à toute heure.
...
```bash
kubectl -n apps-microservices logs deploy/prix-extraction-message --tail=200
kubectl -n apps-microservices logs deploy/prix-extraction-message -f
kubectl -n apps-microservices get pods -l app=prix-extraction-message
...
resource.labels.container_name="prix-extraction-message"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=prix-extraction-message | tail -30
kubectl -n apps-microservices exec -it deploy/prix-extraction-message -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep prix_message_processing
```
...
1. `kubectl -n apps-microservices scale deploy/prix-extraction-message --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=prix-extraction-message --timeout=60s`
3. VM GPU : `docker start rag-hp-pub-prix-extraction-message-1`
4. vérifier `consumers=1` sur `prix_message_processing_queue`
...
| # | Constat | Pour qui |
|---|---|---|
| 1 | Confirmation écrite du dev sur le test `2005786` (28 lignes attendues, présentes une seule fois) | Dev référent |
