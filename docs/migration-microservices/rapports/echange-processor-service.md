...
|---|---|---|
| Où il tourne | `rag-hp-pub-echange-processor-service-1` à `-4`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `echange-processor-service`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 4 | **2** (palier 1 du 24/09), ajustable sur la profondeur de la file |
| File consommée | `echange_processing_queue` sur le broker prod `10.0.1.216` | **La même**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
...
**Ce qui a changé côté configuration** : secret `echange-processor-rabbitmq`, clés `rabbitmq-url`, valeurs dev → **prod** (Secret Manager `platform-rabbitmq-url`, réécrite en DNS interne). Chaque clé posée a été relue par empreinte ; empreintes identiques à celles des conteneurs VM.
...
```bash
kubectl -n apps-microservices logs deploy/echange-processor-service --all-pods --prefix --tail=200
kubectl -n apps-microservices logs deploy/echange-processor-service --all-pods --prefix -f
kubectl -n apps-microservices get pods -l app=echange-processor-service
...
resource.labels.container_name="echange-processor-service"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=echange-processor-service | tail -30
kubectl -n apps-microservices exec -it deploy/echange-processor-service -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep -w echange_processing_queue
```
...
```bash
kubectl -n apps-microservices port-forward deploy/echange-processor-service 8530:8530   # puis http://localhost:8530/metrics
```
...
1. `kubectl -n apps-microservices scale deploy/echange-processor-service --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=echange-processor-service --timeout=60s`
3. VM GPU : `docker start rag-hp-pub-echange-processor-service-1 rag-hp-pub-echange-processor-service-2 rag-hp-pub-echange-processor-service-3 rag-hp-pub-echange-processor-service-4`
