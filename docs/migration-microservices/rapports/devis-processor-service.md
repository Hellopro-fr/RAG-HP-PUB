# Rapport de bascule — `devis-processor-service`
...
|---|---|---|
| Où il tourne | `rag-hp-pub-devis-processor-service-1` à `-4`, Docker Compose, VM `vm-embedding-g2-std-24-use` | Déploiement `devis-processor-service`, namespace `apps-microservices`, cluster `matching-api-dev-k8s` |
| Réplicas | 4 | **2** (palier 1 du 24/09), ajustable sur la profondeur de la file |
| File consommée | `devis_processing_queue` sur le broker prod `10.0.1.216` | **La même**, via `rabbitmq-internal.rabbitmq-v3.svc.cluster.local` (même broker) |
| Sortie | `processed_data_exchange` / `data.ready_for_embedding` (vers `embedding-service`, VM) | **Identique** |
...
**Ce qui a changé côté configuration** : secret `devis-processor-rabbitmq`, clés `rabbitmq-url`, valeurs dev → **prod** (Secret Manager `platform-rabbitmq-url`, réécrite en DNS interne). Chaque clé posée a été relue par empreinte ; empreintes identiques à celles des conteneurs VM.
...
**Il ne touche pas Milvus.** Il prépare la donnée et la publie sur `processed_data_exchange` / `data.ready_for_embedding` (vers `embedding-service`, VM). La suite de la chaîne passe donc par la VM puis revient sur GKE (les écrivains du lot) : un message peut traverser GKE → VM → GKE, via le même broker.
...
```bash
kubectl -n apps-microservices logs deploy/devis-processor-service --all-pods --prefix --tail=200
kubectl -n apps-microservices logs deploy/devis-processor-service --all-pods --prefix -f
kubectl -n apps-microservices get pods -l app=devis-processor-service
...
resource.labels.container_name="devis-processor-service"
```
...
```bash
kubectl -n apps-microservices describe pod -l app=devis-processor-service | tail -30
kubectl -n apps-microservices exec -it deploy/devis-processor-service -- /bin/sh
```
...
kubectl -n rabbitmq-v3 exec "$POD" -- rabbitmqctl list_queues name messages consumers | grep -w devis_processing_queue
```
...
```bash
kubectl -n apps-microservices port-forward deploy/devis-processor-service 8530:8530   # puis http://localhost:8530/metrics
```
...
1. `kubectl -n apps-microservices scale deploy/devis-processor-service --replicas=0`
2. `kubectl -n apps-microservices wait --for=delete pod -l app=devis-processor-service --timeout=60s`
3. VM GPU : `docker start rag-hp-pub-devis-processor-service-1 rag-hp-pub-devis-processor-service-2 rag-hp-pub-devis-processor-service-3 rag-hp-pub-devis-processor-service-4`
...
| 2 | **SIGTERM ignoré** à l'arrêt des conteneurs VM (même défaut que L1-L3, F-HP-DEV-006) | LEAD |
| 3 | Jumeaux VM conservés pendant la fenêtre de rollback, puis retrait (`docker rm` + neutralisation dans `docker-compose.yml`) | DevSecOps |
| 4 | Mot de passe du broker (et de Redis) dans les logs au démarrage (F-HP-SEC-021) — masquer avant partage | Tous |
