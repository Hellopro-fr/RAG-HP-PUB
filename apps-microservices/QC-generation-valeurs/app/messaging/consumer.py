...
class Consumer:
    """Async Consumer en mode streaming pour le service QC-generation-valeurs (step 4)."""
    ...
    async def connect(self):
        ...
        logger.info("✅ QC-Valeurs Consumer initialized (streaming mode)")
    ...
    async def process_message(self, message: AbstractIncomingMessage):
        async with message.process(ignore_processed=True):
            ...
            try:
                ...
                headers = DLQProperties.create_dlq_headers(e, "qc-generation-valeurs", 0, message)
                ...
                if self._is_transient_error(e) and retry_count < MAX_RETRIES:
                    ...
                    headers = DLQProperties.create_dlq_headers(e, "qc-generation-valeurs", retry_count, message)
                    await self.channel.default_exchange.publish(aio_pika.Message(body=message.body, headers=headers, delivery_mode=aio_pika.DeliveryMode.PERSISTENT), routing_key=self.dead_letter_queue_name)
                    await message.ack()
    ...
    async def start_consuming(self):
        await self.connect()
        logger.info(f"👂 QC-Valeurs: En attente sur {self.queue_name} (mode streaming)")
