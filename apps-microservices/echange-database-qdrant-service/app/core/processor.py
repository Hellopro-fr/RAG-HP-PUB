...
def insertion_data(echange_data: dict) -> dict:
    ...
    if not func or len(echanges) <= 0:
        raise ValueError("Aucune donnée d'échange fournie ou fonction de traitement invalide.")
        ...
        if len(data) > 0:
            ...
            if bdd.lower() == "milvus":
                ...
                if result.get("status") == "error":
                    error_message = f"Erreur mise à jour pour {conversation_id}: {result.get('message')}"
                    logging.error(error_message)
                    raise Exception(error_message)
