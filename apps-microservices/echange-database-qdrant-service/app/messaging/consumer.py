...
class Consumer:
    ...
    @measure_processing_time(service_name="echange-database-qdrant-service")
    def _on_message_callback(self, ch, method, properties, body):
        ...
        try:
            ...
            try:
                dlq_props = DLQProperties.create_dlq_properties(e, 'echange-database-qdrant-service', 0, method)
            ...
            try:
                if retry_count < MAX_RETRIES:
                    ...
                    dlq_props = DLQProperties.create_dlq_properties(e, 'echange-database-qdrant-service', MAX_RETRIES, method)
                    ch.basic_publish(exchange=self.dead_letter_exchange, routing_key=self.routing_key, body=body, properties=dlq_props)
                    ch.basic_ack(delivery_tag=method.delivery_tag)
            except Exception:
                logger.warning("Channel closed during error handling, message will be redelivered")
