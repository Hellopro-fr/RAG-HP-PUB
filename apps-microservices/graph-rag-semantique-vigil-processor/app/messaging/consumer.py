...
class Consumer:
    """Consumer for semantic vigil queue."""
    ...
    @measure_processing_time(service_name="graph-rag-semantique-vigil-processor")
    def _on_message_callback(self, ch, method, properties, body):
        try:
            logging.info("📥 Semantic Vigil: Message received")
            ...
            dlq_props = DLQProperties.create_dlq_properties(
                e, "graph-rag-semantique-vigil-processor", 0, method
            )
            ch.basic_publish(
                exchange=self.dead_letter_exchange,
                routing_key=self.routing_key,
    ...
    def start_consuming(self):
        for i in range(3):
            try:
                ...
                logging.info(f"👂 Semantic Vigil listening on: {self.queue_name}")
