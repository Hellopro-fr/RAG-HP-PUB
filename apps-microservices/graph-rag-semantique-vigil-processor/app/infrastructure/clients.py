"""
Service clients for graph-rag-semantique-vigil-processor.
...
from common_utils.grpc_clients import embedding_client as centralized_embedding_client
...
class EmbeddingClientHTTP:
    """HTTP client for Embedding Service (Temporary)."""
...
class EmbeddingClientGRPC:
    """gRPC client for Embedding Service using centralized client."""

    def __init__(self):
        logging.info(
            f"EmbeddingClientGRPC initialized for {settings.EMBEDDING_SERVICE_URL}"
        )

    def get_embedding(self, text: str) -> List[float]:
        """Get embedding for a single text (sync wrapper)."""
        try:
            return _run_async(centralized_embedding_client.get_embedding(text))
        except Exception as e:
            logging.error(f"Embedding RPC error: {e}")
            return []
...
class MilvusClient:
    ...
    def __init__(self):
        logging.info(
            f"MilvusClient initialized for {settings.MILVUS_SERVICE_URL}"
        )
    ...
    def check_entities_exist(self, ids: List[str]) -> List[str]:
        ...
        try:
            return _run_async(
                graph_milvus_client.check_entities_exist(ids=ids)
            )
        except Exception as e:
    ...
    def check_labels_exist(self, labels: List[str]) -> List[str]:
        ...
        try:
            return _run_async(
                graph_milvus_client.check_labels_exist(labels=labels)
            )
        except Exception as e:
...
# Singleton instances
# embedding_client = EmbeddingClientGRPC()
