...
from app.core.processor import prepare_fournisseur_statements, get_graph_id_from_data
...
class Consumer:
    """Async Consumer for Fournisseur Processor with Parallel Batch Workers and Deadlock Prevention."""
    ...
    async def setup(self):
        ...
        logging.info("✅ Fournisseur Consumer initialized")
    ...
    async def _process_batch(self, batch: list, worker_id: int):
        """
        Execute a batch of Fournisseur messages with Deadlock Prevention.
        ...
        for msg in batch:
            try:
                ...
                if not data:
                    logging.warning("Empty data in Fournisseur message, skipping")
                    asyncio.create_task(msg.ack())
                    continue

                # Get graph_id for deadlock detection
                graph_id = get_graph_id_from_data(data)

                # Generate list of statements for this single message
                msg_statements = prepare_fournisseur_statements(data, database, origin)
                ...
                # Add to valid items
                valid_items.append((msg, msg_statements, graph_id))

            except Exception as e:
                logging.error(f"Error preparing Fournisseur message: {e}")
                headers = DLQProperties.create_dlq_headers(
                    e, "graph-rag-fournisseur-processor", 0, msg
                )
        ...
        if no_deadlock_items:
            ...
            if batch_success:
                logging.info(
                    f"✅ Worker {worker_id}: Fournisseur Batch executed successfully ({len(batch_statements)} statements, {len(no_deadlock_items)} items)"
                )
                logging.info(
    ...
    async def start_consuming(self):
        ...
        logging.info(
            f"👂 Fournisseur Consumer listening on: {self.queue_name} with {settings.MAX_CONCURRENCY} parallel batch workers"
        )
