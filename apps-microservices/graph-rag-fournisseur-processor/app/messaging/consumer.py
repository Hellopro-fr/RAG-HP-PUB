...
class Consumer:
    """Async Consumer for Fournisseur Processor with Batch Processing and Deadlock Prevention."""
    ...
    async def _process_batch(self, batch: list):
        ...
        if no_deadlock_items:
            ...
            if batch_success:
                logging.info(
                    f"✅ Fournisseur Batch executed successfully ({len(batch_statements)} statements, {len(no_deadlock_items)} items)"
                )
                logging.info(
    ...
    async def start_consuming(self):
        ...
        logging.info(f"👂 Fournisseur Consumer listening on: {self.queue_name}")
