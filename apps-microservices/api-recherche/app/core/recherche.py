...
from app.schemas.search import (
    LLMPipeline,
    SearchReponse,
    SearchRequestWs as SearchRequest,
    SearchResponse,
)
...
def llm_prompt_stream(request: SearchRequest, context_texts):
    ...
    context = "\n-----\n\n\n".join(context_texts)
    try:
        full_user_prompt = request.llm.template_prompt.format(
            chunks=context, recherche=request.prompt
        )
    ...
    type_prompt = next(
        (
            ...
            if request.llm.chat_model in values
        ),
        "openai",
    )
...
async def filtre_source(filtre: dict, source: str = "") -> list:
    ...
    if not field_types:
        logger.warning(
            f"Impossible de récupérer le schéma pour la collection '{source}'. Le filtrage sera ignoré pour cette source."
        )
        return []
    NUMERIC_DTYPES = {
        DataType.INT8.value,
        ...
        if dtype == DataType.ARRAY:
            ...
            if isinstance(val, dict):
                ...
                numeric_val = (
                    int(val)
                    if dtype
                    in {DataType.INT8, DataType.INT16, DataType.INT32, DataType.INT64}
                    else float(val)
                )
...
def context(matches: List[Dict[str, str]]):
    context_texts = []
    for res in matches:
        categorie = (
            res["metadata"]["entity"]["categorie"]
            if res["metadata"]["entity"]["categorie"]
            else "N/A"
        )
        source = res["source"]
        fournisseur = "N/A"
        ...
        if source == "produits_3":
            title = (
                res["metadata"]["entity"]["nom_produit"]
                if res["metadata"]["entity"]["nom_produit"]
                else title
            )
            source = "Produits"
            fournisseur = (
                res["metadata"]["entity"]["fournisseur"]
                if res["metadata"]["entity"]["fournisseur"]
                else "N/A"
            )
        elif source == "siteweb_2":
            title = (
                res["metadata"]["entity"]["url"]
                if res["metadata"]["entity"]["url"]
                else title
            )
            source = "Siteweb"
            fournisseur = (
            ...
            title = (
                res["metadata"]["entity"]["lead_id"]
                if res["metadata"]["entity"]["lead_id"]
                else title
            )
            ...
            title = (
                res["metadata"]["entity"]["conversation_id"]
                if res["metadata"]["entity"]["conversation_id"]
                else title
            )
...
async def search_in_milvus_stream(request: SearchRequest):
    ...
    try:
        ...
        yield {
            "type": "status",
            "payload": "Génération de l'embedding pour la requête...",
        }
        ...
        if not query_vector:
            yield {
                "type": "error",
                "payload": "Impossible de générer l'embedding pour la requête.",
            }
            return
        ...
        top_k_retrieval = (
            top_k_final * 2 if request.options.use_reranker else top_k_final
        )
        ...
        for item in request.source:
            ...
            yield {
                "type": "status",
                "payload": f"Préparation de la recherche pour '{source_name}'...",
            }
            ...
            async def create_search_task(s_name=source_name, s_filtre=filtre):
                ...
                filter_expr_source = (
                    await filtre_source(s_filtre, s_name) if s_filtre else ""
                )
        ...
        yield {
            "type": "status",
            "payload": f"Lancement de la recherche parallèle sur {len(search_tasks)} source(s)...",
        }
        ...
        initial_matches = sorted(
            all_source_results, key=lambda x: x["score"], reverse=True
        )
        ...
        yield {
            ...
            "payload": {
                "results": initial_matches[:top_k_final],
        ...
        if request.options.use_reranker and initial_matches:
            yield {
                "type": "status",
                "payload": "Reclassement (reranking) des résultats...",
            }
            ...
            if not docs_to_rerank:
                yield {
                    "type": "status",
                    "payload": "Reranking annulé: aucun texte trouvé dans les résultats.",
                }
            else:
                yield {
                    "type": "status",
                    "payload": f"Reclassement de {len(docs_to_rerank)} documents...",
                }
                start_rerank_time = time.perf_counter()

                # Appel au microservice avec une charge utile minimale
                ranked_texts = await reranking_client.rerank_documents(
                    request.prompt, docs_to_rerank
                )
        ...
        if request.action == 2 and final_results:
            yield {
                "type": "status",
                "payload": f"Génération de la réponse avec le LLM...",
            }

            # Préparation du contexte pour le LLM
            context_texts = context(final_results[:top_k_final])
            ...
            token_generator = await asyncio.to_thread(
                llm_prompt, request, context_texts
            )
            # token_generator = llm_prompt(request, context_texts)
            yield {
                "type": "llm_chunk" if not token_generator.error else "error",
                "payload": token_generator.llm_response,
                "llm_response": token_generator.response,
            }
        ...
        logger.error(
            f"Une erreur majeure est survenue dans le flux de recherche: {e}",
            exc_info=True,
        )
...
async def search_in_milvus(request: SearchRequest) -> dict:
    ...
    """
    logger.info(
        f"[gRPC] Recherche (non-stream): prompt='{request.prompt[:50]}...', sources={[s.source for s in request.source]}"
    )
    start_total_time = time.perf_counter()
    ...
    try:
        ...
        all_results = (
            {}
        )  # Dictionnaire pour stocker les résultats par source, comme l'original
        ...
        for item in request.source:
            ...
            filter_expr_source = (
                await filtre_source(filtre, source_name) if filtre else []
            )
            if filter_expr_source:
                filters.append(" and ".join(filter_expr_source))
            ...
            logger.info(
                f"Recherche dans '{source_name}' avec le filtre: {final_filter_expr}"
            )
        ...
        if request.options.use_reranker and all_results:
            ...
            for source, matches in all_results.items():
                ...
                reranked_results_by_source[source] = reranked_results_by_source[source][
                    :top_k_final
                ]
                logging.info(
        ...
        for source, matches in all_results.items():
            ...
            context_texts.extend(context(matches))
        ...
        logger.error(
            f"Une erreur majeure est survenue dans la recherche (non-stream): {e}",
            exc_info=True,
        )
...
async def search_in_milvus_classique_stream(request: SearchRequest):
    ...
    try:
        ...
        for item in request.source:
            ...
            yield {
                "type": "status",
                "payload": f"Recherche classique dans '{source_name}'...",
            }
            ...
            if source_results is None:
                yield {
                    "type": "warning",
                    "payload": f"Erreur lors de la recherche dans '{source_name}'.",
                }
                continue
        ...
        if request.action == 2 and final_results:
            yield {
                "type": "status",
                "payload": "Génération de la réponse avec le LLM...",
            }
            # context_texts = [res['metadata']['entity']['text'] for res in final_results]
            context_texts = context(final_results)
        ...
        logger.error(
            f"Erreur majeure dans le flux de recherche classique: {e}", exc_info=True
        )
        yield {"type": "error", "payload": f"Erreur serveur: {e}"}
    finally:
...
async def search_in_milvus_classique(request: SearchRequest) -> dict:
    ...
    """
    logger.info(
        f"[gRPC] Recherche classique (non-stream): filtre='{request.filtre}', sources={[s.source for s in request.source]}"
    )
    start_total_time = time.perf_counter()
    ...
    try:
        ...
        for item in request.source:
            ...
            logger.info(
                f"Recherche classique dans '{source_name}' avec filtre: {final_filter_expr}"
            )
        ...
        logger.error(
            f"Erreur majeure dans la recherche classique (non-stream): {e}",
            exc_info=True,
        )
        return {
            "database": "milvus",
