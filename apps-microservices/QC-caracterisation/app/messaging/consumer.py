...
from app.core.caracterisation_produit import CaracterisationProduit
...
class Consumer:
    """Consumer pour le service QC-caracterisation (step 7)."""
    def __init__(self, connection: pika.BlockingConnection, publisher: Publisher):
        ...
        self.routing_key = 'qc.step7.start'
        self.queue_name = 'qc_caracterisation_queue'
        ...
        self.connect()
        print("✅ QC-Caracterisation Consumer initialisé.")
    ...
    def _send_to_dlq(self, ch, method, body, error: Exception, retry_count: int):
        ...
        try:
            dlq_props = DLQProperties.create_dlq_properties(error, 'qc-caracterisation', retry_count, method)
    ...
    def _on_message_callback(self, ch, method, properties, body):
        ...
        try:
            print("📥 QC-Caracterisation: Message reçu.")
            ...
            print(f"\n📥 QC-Caracterisation: Traitement catégorie '{id_categorie}'.")
            
            request = RequestProcessus(id_categorie=id_categorie, is_reset=is_reset)
            api_client = HelloProAPIClient()
            generator = CaracterisationProduit(api_client)
            
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                result = loop.run_until_complete(generator.generate_all_caracterisations(request))
            ...
            active_ch = self._ensure_channel_open(ch)
            active_ch.basic_ack(delivery_tag=method.delivery_tag)
            print(f"✅ QC-Caracterisation: Catégorie '{id_categorie}' traitée.")
    ...
    def start_consuming(self):
        ...
        for i in range(3):
            try:
                ...
                print("👂 QC-Caracterisation: En attente de messages...")
