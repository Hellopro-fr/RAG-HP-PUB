...
class Consumer:
    """Async Consumer en mode streaming pour le service prix-extraction-message."""
    ...
    def __init__(self, publisher: Publisher):
        ...
        # Nomenclature centralisée — CollectionName.PRIX_MESSAGE
        _collection          = CollectionName.PRIX_MESSAGE          # "prix_message"
        self.exchange_name   = f'data_exchange_{_collection}'       # data_exchange_prix_message
        self.routing_key     = RoutingKeys[_collection]             # new_data.prix_message
        self.queue_name      = f'{_collection}_processing_queue'    # prix_message_processing_queue
        self.retry_exchange  = f'retry_exchange_{_collection}'      # retry_exchange_prix_message
    ...
    async def connect(self):
        ...
        logger.info("✅ Prix-Extraction-Message Consumer initialized (streaming mode)")
    ...
    async def process_message(self, message: AbstractIncomingMessage):
        async with message.process(ignore_processed=True):
            ...
            try:
                ...
                logger.info(f"[CAT-{id_categorie}] 📥 Début traitement extraction prix message")
                            ...
                            for single_prix in item_result.prix_data:
                                try:
                                    embedding_message = process_product_data_for_embedding(
                                        ...
                                        origin="prix-extraction-message"
                                    )
                ...
                headers = DLQProperties.create_dlq_headers(e, "prix-extraction-message", 0, message)
                ...
                if self._is_transient_error(e) and retry_count < MAX_RETRIES:
                    ...
                    headers = DLQProperties.create_dlq_headers(e, "prix-extraction-message", retry_count, message)
                    await self.channel.default_exchange.publish(aio_pika.Message(body=message.body, headers=headers, delivery_mode=aio_pika.DeliveryMode.PERSISTENT), routing_key=self.dead_letter_queue_name)
                    await message.ack()
    ...
    async def start_consuming(self):
        await self.connect()
        logger.info(f"👂 Prix-Extraction-Message: En attente sur {self.queue_name} (streaming)")
