...
// AdminServer exposes non-MCP REST endpoints used by internal administration
// UIs to list Leexi users and derive teams. Protected by a shared secret so
// the endpoints are never reachable from the public internet through
// misconfiguration.
...
// NewAdminServer returns an AdminServer bound to the given Leexi client. An
// empty token disables the endpoints entirely.
func NewAdminServer(client *leexi.Client, token string) *AdminServer {
...
func (s *AdminServer) handleUsers(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, map[string]any{"users": users, "count": len(users)})
}
...
func (s *AdminServer) handleTeams(w http.ResponseWriter, r *http.Request) {
	...
	teams := leexi.TeamsFromUsers(users)
	writeJSON(w, map[string]any{"teams": teams, "count": len(teams)})
}
