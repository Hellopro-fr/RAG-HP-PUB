    ...
    async def stream_chat(self, message_history, temperature: float, max_tokens: int, enable_thinking: bool):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logging.warning(f"Tentative {attempt + 1}/{MAX_RETRIES} échouée pour contacter vLLM (stream): {e}")
                if attempt + 1 == MAX_RETRIES:
                    logging.error(f"Échec final pour contacter vLLM (stream) après {MAX_RETRIES} tentatives.")
    ...
    async def get_chat_completion(self, message_history, temperature: float, max_tokens: int, enable_thinking: bool) -> str:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logging.warning(f"Tentative {attempt + 1}/{MAX_RETRIES} échouée pour contacter vLLM: {e}")
                if attempt + 1 == MAX_RETRIES:
