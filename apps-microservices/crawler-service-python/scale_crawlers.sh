...
# NOTE: Using LOCAL docker-compose.yaml in crawler-service-python/
docker compose -f apps-microservices/crawler-service-python/docker-compose.yaml --profile crawling up -d --no-deps reverse-proxy
...
# NOTE: Adjusted for Python service and local compose file
docker compose -f apps-microservices/crawler-service-python/docker-compose.yaml --profile crawling up -d --no-deps --scale crawler-service=$REPLICAS
echo ""

# --- PHASE 5: Reload Nginx to discover new replicas ---
echo "[PHASE 5/5] Reloading Nginx in reverse-proxy..."
# reload nginx
docker compose -f apps-microservices/crawler-service-python/docker-compose.yaml --profile crawling exec reverse-proxy nginx -s reload
echo "Nginx reloaded."
echo ""
