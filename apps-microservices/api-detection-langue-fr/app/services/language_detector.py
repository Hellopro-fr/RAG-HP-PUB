...
def detect_challenge_page(html: str) -> Optional[str]:
    ...
    # en français (proxy configuré en FR) et provoquent des faux positifs sur les
    # sites .fr injoignables (ex: www.podia.fr → Squid error FR → NLP détecte FR).
    import re as _re_squid
    # body id="ERR_*" est EXCLUSIF aux pages d'erreur Squid — jamais sur un vrai site
    if _re_squid.search(r'<body[^>]*id="err_', html_lower):
        return 'Squid_proxy_error'
    squid_indicators = [
        'squid-internal-static',                       # Ressources internes Squid
        'the squid software foundation',               # Copyright Squid
        'par localhost (squid)',                        # Footer Squid (FR)
        'by localhost (squid)',                         # Footer Squid (EN)
        'generated by squid',                          # Footer Squid variante
    ]
    if sum(1 for p in squid_indicators if p in html_lower) >= 1:
        return 'Squid_proxy_error'
