...
| `TERMINAL_WRITE_BUDGET_S` | `60` | Deadline for the terminal-write retry loop, clamped to the remaining `JOB_MAX_S` (see hardening note above). `60` = 2× `REDIS_HEALTH_CHECK_INTERVAL_S`/`REDIS_RECONNECT_INTERVAL_S` (30s each) — at least one full healing cycle of either pool mechanism — and half of `STALE_THRESHOLD_S`. |
