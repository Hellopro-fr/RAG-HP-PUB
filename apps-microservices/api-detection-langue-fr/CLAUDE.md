...
### No Verdict Without Text (N1 guard)

**Rule: a NEGATIVE French verdict can never be concluded from a page whose visible text is empty.** Scoped deliberately to the negative side only — a POSITIVE FR conclusion drawn from URL or HTML metadata alone stays legitimate without text (that's what the `nlp_skipped` methods already are). Widening this to positive verdicts would cut real FR detections for no benefit.

Diagnosed 2026-08-13 from a production `/detect-debug` probe (`324automatismes.net`): `fetch.status_code=202`, `cleaning.cleaned_text_length=0`, `nlp.available=false`, yet the published verdict was `Check_nok_v2` ("Non-FR jugé") — a negative linguistic judgment drawn from zero characters of text. The page was an AWS WAF challenge (see N2 below), compounding the defect: the challenge went undetected, so its empty body reached the decision matrix as if it were real content.

`DomainFR._extract_visible_text_coarse` (`app/core/domain_fr.py`) computes visible text ONCE, early in `check_page_if_french`, with the same coarse stripping (`script`/`style`/`meta`/`link`/`noscript` decomposed, then `get_text`) the pre-existing `.fr`-TLD guard (Case 2b) already used — deliberately coarser than `LanguageDetector.clean_html_to_text` (which also strips `head`/`img`/`svg`/`iframe`/... and cookie-consent banners). Below `NLP_MIN_TEXT_LENGTH`, the three methods the BO's own `DETECTION_LANGUAGE_VERDICTS` allowlist (`pct_traitement_crawling_rindra_BO_args.php:40`) treats as "judged" negative verdicts are replaced by `fetch_empty_content` instead:

| Would-be method | Case | Guard added |
|---|---|---|
| `nlp_override_tld_fr` | 2a — TLD `.fr` but NLP contradicts | 2026-08-13 |
| `nlp_not_confirmed` | 7 — HTML/URL indicators but NLP disagrees | 2026-08-13 |
| `Check_nok_v2` | 9 — no indicator at all | 2026-08-13 (was the sole guard before; Case 2b's `.fr` variant predates this and was already guarded) |

No new `method` value, no new setting, no contract change: `fetch_empty_content` was already classified indeterminate by the BO and already in `_PASS2_RETRYABLE_METHODS`. **Consequence:** `fetch_empty_content` is also in `_VARIANT_RESCUE_METHODS` (`app/api/routes.py`) — domains that used to land on `Check_nok_v2` now route into the post-verdict variant rescue, adding browser probes per run (see "Post-verdict rescue" below). Not compensated for; watch it against admission/browser-pool load like any other rescue-population change.

Case 8 (last-resort lexical signal) and the Volet B lexical census at Case 9 are untouched — this guard never fires ahead of an `ok=True` return. Case 9's `alternative_urls` stays empty on the `fetch_empty_content` guard too (not just on `Check_nok_v2`) — the same crawler/BO contract documented at Case 9 above applies regardless of which method Case 9 ends up returning; Cases 2a and 7 have no such contract and keep passing `alternative_urls` through.

**What N1's live population actually is.** `detect_challenge_page` (N2 above) runs in `app/api/routes.py` *before* `check_page_if_french` is ever called — a detected challenge returns `challenge_page` directly, and the decision matrix never sees the page. So the motivating incident itself, once N2 also ships, resolves as `challenge_page` (retryable, ~45s of browser polling since an AWS CAPTCHA doesn't self-resolve, plus the Pass-2 retry — roughly 90s per affected domain per run), not `fetch_empty_content`. N1's actual live population is textless pages that match **no known challenge signature** — a narrower set than the motivating example suggested. `check_page_if_french`'s own guard (this section) still matters standalone: it's what protects any caller that reaches the decision matrix without having run challenge detection first (e.g. a future caller, or the Case-6 alternative-confirmation loop, which does run its own `detect_challenge_page` check but on the *alternative's* content, not the primary page already past this guard).

**Crawler-service path.** `crawler/src/routes.ts` computes its own `nlpRejected` flag from `nlp_not_confirmed`/`nlp_override` — it does not check for `fetch_empty_content`. So a domain that used to fail one of those two now returns `fetch_empty_content` instead, `nlpRejected` stays `false`, and the crawler's URL-only `checkUrl` fallback re-engages: a `.fr`/`/fr/` page the API couldn't read gets accepted as French by URL and the crawl proceeds. Safe direction for this incident class (a page that couldn't be read no longer gets a confident negative), but worth knowing if a crawler-side audit ever asks why a textless page kept crawling.
...
**Per-copy size, MEASURED on the VM 2026-08-14** (`ls -l /mnt/data/models/fasttext/`): the deployed model is the full `lid.176.bin` at **131 266 198 bytes = 125.2 MiB**, sole file in the mount, dated `Jan 18 2019` (upstream release date — identity corroborated). The compressed `lid.176.ftz` (<1 MB) is NOT what runs here. The path is the hardcoded default (no `FASTTEXT_MODEL_PATH` setting exists) and compose mounts `/mnt/data/models/fasttext:/app/models`; the model is not baked into the image, so the file is deployment-invariant.

**Fix VERIFIED in production 2026-08-14**, on a freshly restarted container (RSS 282 MB, zero validation verdicts, idle): a first `/detect-debug` that reached NLP took RSS to 335.3 MB; two further probes that BOTH ran fastText (`nlp.method = "nlp_detection_fasttext"`, confirmed in the debug payload — not `langdetect`) added **0 MB then +2 MB**. Under the old per-instance code each would have added another copy. Per-call duplication is gone.

**Resident cost per copy is NOT the file size** — do not conflate them, an earlier version of this note did. The file is 125.2 MiB, but the first load added only ~53 MB of RSS, and that delta *also* covers the first-scrape import of camoufox/browserforge (the Bayesian fingerprint networks, order 10–60 MB, module-scoped so one-time). The model's own resident share is therefore **≤ ~53 MB**, materially below its on-disk size — plausibly because only the matrix pages actually touched by predictions become resident, but that mechanism is **[UNCLEAR]** and unverified here. Consequence: the saving at 8-way concurrency is bounded ABOVE by ~424 MB, not the ~1.0 GiB that 8 × 125.2 MiB suggests. Counter-caveat in the other direction: a real batch analyses diverse texts and touches more of the 2M-bucket matrix than one repeated page, so per-copy residency under production load is likely higher than this probe's. The honest range is "somewhere between ~53 MB and ~125 MiB per copy"; pin it with per-PID `RssAnon` during a real batch, not with `ls`.

The disk-read cost is unaffected by that uncertainty and is the firmer half of the win: roughly **12–19 GiB of synchronous reads per 98-domain run** (~100–150 loads × 125 MiB) issued from the event-loop thread, now reduced to one read.

**This does not close the 2026-08-13 OOM.** uvicorn's 1334 MiB was only ~30% of the then-4500m cgroup, and victim selection identifies the largest *task*, never the largest *share* (see the `docker-compose.yml` correction note) — ~3.1 GB of that cgroup remains unattributed and no Python-side finding accounts for it. Note also that the once-quoted "1013 MiB observed vs 1008 MB predicted" agreement stays **invalid** regardless of this measurement: it subtracted a VmRSS from a dmesg `anon-rss`, two different sets. Confirming the per-copy unit settles the magnitude of THIS fix; it does not repair that arithmetic.
...
**Pass 2 bounded on the remaining job budget (2026-08-13 incident).** Sequential retries at up to 300s + 2s each can, on their own, exceed `JOB_MAX_S` (incident: Pass 1 took 328s, 4 sequential retries pushed the job to ~1536s > 1500s) — the worker watchdog (`_abandon_job`, `app/core/async_jobs.py`) then abandons the WHOLE job, discarding every item's result, including ones that had already succeeded in Pass 1. `_run_batch_core` takes an optional `deadline_monotonic` (an absolute `time.monotonic()` deadline); the async worker computes it from its own job start + `JOB_MAX_S` and passes it down (`JobManager._run_job`). **Both** Pass-2 retry loops — the sequential one (`complete`/`simple`) and the grouped one (`first_match`, reachable from the async worker too: `AsyncBatchSubmitRequest.mode` accepts any mode and is forwarded verbatim) — check, before starting each retry, that at least `_ITEM_WALL_CLOCK_S + _PASS2_BUDGET_MARGIN_S` (`300 + 15`) remains; a retry started with less would get cancelled mid-navigation once the watchdog fires, the same orphaned-callback condition the `_MIN_PROBE_S` floor guards against in the variant rescue. Both **stop retrying** (never cancel a retry already in flight) once the check fails. Skipped items keep their Pass 1 verdict untouched (still retryable on the BO's next run, not turned into a hard failure). `deadline_monotonic=None` (the sync `/detect-batch` route's call shape, no `JOB_MAX_S` to bound itself against) reproduces today's unconditional-retry behavior exactly. Skips are logged (`[BATCH] Pass 2: budget de job insuffisant...` / `[BATCH][first_match] Pass 2: ...`) and counted (`detection_batch_pass2_retry_skipped_total`). Tests: `tests/test_pass2_budget.py`.
...
- `detect_teardown_abandoned_total{op}` — counter. Browser teardowns abandoned after `TEARDOWN_TIMEOUT_S` by `_close_or_abandon` (`app/services/scraper.py`). `op` ∈ `unroute_all`/`context.close`/`browser.close`/`playwright.stop`, derived from the call site's `what` string with the **URL deliberately stripped** (unbounded cardinality). Added 2026-08-17: before it, an abandon left exactly one WARNING line and no counter, so its real frequency was unknowable.
- `detect_browsers_unclosed` — gauge. Browsers launched (`_launch_browser` call sites in `scrape_html` and `scrape_html_with_redirects`) whose `close()` has not returned. Decremented **only** when a `browser.close` task settles WITHOUT RAISING — including long after it was abandoned — inside `_drain_orphan_exception`. **Not a process count**: this service tracks no PID and kills nothing, so "the browser exited" is only ever inferred from `close()` returning. THREE paths deliberately leave a browser counted forever, and they are the same fact seen three ways — a dead driver pipe is no proof the DETACHED Firefox exited: an abandoned close; a teardown where `browser.is_connected()` was already false so `close()` was never attempted; and a close that settled by RAISING (typically `TargetClosedError`), since `close()` only returns after the driver confirms the browser dead and its profile removed.

**Under-reports on one shape, by construction:** the gauge counts browsers *returned* by `_launch_browser`. A Camoufox launch that exceeds `BROWSER_LAUNCH_TIMEOUT_S` is cancelled and falls back to Chromium, so that attempt is never counted even though the cancelled Firefox may survive. The gauge therefore under-reports precisely on the launch-timeout incident shape that motivated the pool sizing — do not read a low value as "no accumulation". Sustained values above `BROWSER_SEMAPHORE_SIZE` therefore read as "browsers accumulating faster than teardowns settle" — the overlap the abandon mechanism creates. **How many abandoned browsers actually survive has never been measured**; do not read a process count off this gauge.
  Replaced `detect_browser_semaphore_waiters` (deleted 2026-08-17): defined since 2026-04-20 and written from nowhere, so it read a constant `0` — "no queue" rather than "no data", a lie an operator cannot detect. Grafana panels or PromQL referencing it go silent; the two 2026-04-20/2026-05-17 spec+plan documents naming it are dated records and were left as-is.
