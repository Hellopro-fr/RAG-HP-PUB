...
// Table is a single table descriptor. Extra is reserved for forward
// compatibility — the catalog payload may grow extra fields, and a future
// custom UnmarshalJSON could capture them. For v1 the field is intentionally
// left empty by the decoder; consumers should treat it as a marker only.
type Table struct {
	ID          int                        `json:"id"`
...
// Field describes one column of a table. Extra plays the same forward-compat
// role as on Table.
type Field struct {
	ID          int                        `json:"id"`
