...
def main():
    ...
    for i in range(10):
        try:
            ...
            print("✅ webhook-Processor: Connecté à RabbitMQ.")
            break
        except pika.exceptions.AMQPConnectionError:
            print(f"⏳ webhook-Processor: En attente de RabbitMQ... {i+1}s")
            time.sleep(1)

    if not connection:
        print("❌ webhook-Processor: Impossible de se connecter, arrêt du service.")
        exit(1)
    ...
    try:
        ...
        print("\n🛑 webhook-Processor: Arrêt demandé.")
    finally:
        if connection and not connection.is_closed:
            connection.close()
            print("✅ webhook-Processor: Connexion RabbitMQ fermée.")
