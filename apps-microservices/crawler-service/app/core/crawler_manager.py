...
class CrawlerManager:
    ...
    async def start_crawl(self, domain: str, start_url: str, crawl_id_prefix: str, callback_url: str, params: Dict[str, Any]) -> str:
        ...
        crawl_id = f"{crawl_id_prefix}-{uuid.uuid4()}"
        job_storage_path = os.path.join(settings.CRAWLER_STORAGE_PATH, crawl_id)
        
        try:
            os.makedirs(job_storage_path, exist_ok=True)
            logger.info(f"Created storage for crawl_id '{crawl_id}' at '{job_storage_path}'")
        except OSError as e:
            logger.error(f"Failed to create storage directory for crawl '{crawl_id}': {e}")
    ...
    async def stop_crawl(self, crawl_id: str) -> bool:
        ...
        stopper_dir = os.path.join(storage_path, 'stopper')
    ...
    async def shutdown(self):
        ...
        # Create stop tasks
        stop_tasks = [self.stop_crawl(crawl_id) for crawl_id in self.active_crawls]
        ...
        for crawl_id, job_info in self.active_crawls.items():
