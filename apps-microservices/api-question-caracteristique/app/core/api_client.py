...
class HelloProAPIClient:
    ...
    BASE_URL = "https://bo.hellopro.fr/generation_question/index.php"
    ...
    async def post(self, etape: str, field: str, action: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            url = self.BASE_URL
            response = await self.client.post(url, json={"etape": etape, "field": field, "action": action, "data": data})
            ...
            if http_code == 200:
                ...
                logger.error(f"Erreur HTTP sur {action}: {http_code} {response}")
                return None
        except httpx.HTTPError as e:
            logger.error(f"Erreur HTTP sur {action}: {e}")
            return None
        except Exception as e:
            logger.error(f"Erreur lors de l'appel API {action}: {e}")
            return None