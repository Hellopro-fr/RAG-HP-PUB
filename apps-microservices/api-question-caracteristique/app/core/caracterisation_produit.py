...
class CaracterisationProduitGenerator:
    ...
    def _create_id_mapping(
        ...
        for idx, carac in enumerate(caracteristiques, 1):
            ...
            # Créer le mapping
            id_mapping[idx] = id_base
            reverse_mapping[str(id_base)] = idx
    ...
    async def caracterise_produit(
        ...
        # Récupérer le prompt
        prompt_config = await utils.get_prompt(self.PROMPT_CARACTERISATION_ID)
        
        if not prompt_config:
            self._log("ERREUR: Impossible de récupérer le prompt Caractérisation")
            raise Exception("Impossible de récupérer le prompt Caractérisation")
    ...
    async def repasse_caracterisation(
        ...
        # Récupérer le prompt de repasse
        prompt_config = await utils.get_prompt(self.PROMPT_REPASSE_ID)
        
        if not prompt_config:
            self._log("ERREUR: Impossible de récupérer le prompt Repasse")
            raise Exception("Impossible de récupérer le prompt Repasse")
        ...
        if "code" in result:
            ...
            raise Exception(f"Erreur API Gemini repasse: {result.get('error')}")
        
        # Extraire le JSON
        response_text = result.get("message", "").strip()
        self._log(f"Réponse LLM repasse: {response_text[:500]}...")
    ...
    async def generate_all_caracterisations(
        ...
        description_categorie = category_info.get("description_categorie", "")
        ...
        jeu_carac_dict = {carac.get('id'): carac for carac in jeu_caracteristique}
