...
def _append_manifest_logo_entry(domain: str, entry: dict) -> None:
    """
    Ajoute ou remplace une entree dans ``manifest_logo.json`` sous lock NFS.

    Dedup sur ``entry["key"]`` (contrairement au manifest pages, dedup sur
    url_source) : une nouvelle ingestion du meme logo fournisseur (MAJ) remplace
