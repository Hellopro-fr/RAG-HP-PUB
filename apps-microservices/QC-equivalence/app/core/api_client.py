    ...
    async def post(self, etape: str, field: str, action: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if http_code == 200:
                ...
                logger.error(f"Erreur HTTP sur {etape} {field} {action} : {http_code} {response.text}")
                return None
        except httpx.HTTPError as e:
            logger.error(f"Erreur HTTP sur {etape} {field} {action} : {e} {response.text}")
            return None
        except Exception as e:
            logger.error(f"Erreur lors de l'appel API {etape} {field} {action} : {e} {response.text}")
            return None
