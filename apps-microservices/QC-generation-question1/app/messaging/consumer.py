...
from app.core.question1_generator import Question1Generator
...
class Consumer:
    """Async Consumer en mode streaming pour le service QC-generation-question1 (step 1).
    
    Mode streaming: Les messages sont traités immédiatement dès leur arrivée,
    sans attendre la fin du batch précédent. Le semaphore contrôle la concurrence.
    """
    ...
    def __init__(self, publisher: Publisher):
        ...
        self._processing_categories: Set[str] = set()  # Pour déduplication en temps réel
        self._categories_lock = asyncio.Lock()

        self.exchange_name = 'qc_pipeline_exchange'
        self.routing_key = 'qc.step1.start'
        self.queue_name = 'qc_question1_queue'
    ...
    async def connect(self):
        ...
        logger.info("✅ QC-Question1 Consumer initialized (streaming mode)")
    ...
    async def _is_duplicate_category(self, cat_id: str) -> bool:
        """Vérifie si une catégorie est déjà en cours de traitement (déduplication temps réel)."""
    ...
    async def _release_category(self, cat_id: str):
        """Libère une catégorie après traitement."""
        async with self._categories_lock:
        ...
        async with message.process(ignore_processed=True):
            ...
            try:
                ...
                try:
                    ...
                    generator = Question1Generator(api_client)
                    ...
                    output_message = {'id_categorie': id_categorie, 'is_reset': is_reset, 'step': 2, 'previous_step': 'question1', 'status': result.status}
                ...
                headers = DLQProperties.create_dlq_headers(e, "qc-generation-question1", 0, message)
                ...
                if self._is_transient_error(e) and retry_count < MAX_RETRIES:
                    ...
                    headers = DLQProperties.create_dlq_headers(e, "qc-generation-question1", retry_count, message)
                    await self.channel.default_exchange.publish(aio_pika.Message(body=message.body, headers=headers, delivery_mode=aio_pika.DeliveryMode.PERSISTENT), routing_key=self.dead_letter_queue_name)
                    await message.ack()

    async def _process_with_semaphore(self, message: AbstractIncomingMessage):
        """Wrapper qui gère le semaphore et le nettoyage des tâches."""
    ...
    async def start_consuming(self):
        ...
        await self.connect()
        logger.info(f"👂 QC-Question1: En attente sur {self.queue_name} (mode streaming)")
        logger.info(f"🚀 Configuration: max_concurrency={settings.MAX_CONCURRENCY}")

        async with self.queue.iterator() as queue_iter:
            async for message in queue_iter:
                # Lance le traitement immédiatement sans attendre
