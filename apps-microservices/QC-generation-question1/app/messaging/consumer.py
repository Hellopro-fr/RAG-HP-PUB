...
class Consumer:
    """
    Consumer pour le service QC-generation-question1.
    Écoute la queue step1, traite les données, et publie vers step2.
    """
    def __init__(self, connection: pika.BlockingConnection, publisher: Publisher):
        ...
        self.connect()
        print("✅ QC-Question1 Consumer initialisé.")
    ...
    def _send_to_dlq(self, ch, method, body, error: Exception, retry_count: int):
        ...
        try:
            dlq_props = DLQProperties.create_dlq_properties(error, 'qc-generation-question1', retry_count, method)
    ...
    def _on_message_callback(self, ch, method, properties, body):
        ...
        """
        try:
            print("📥 QC-Question1: Message reçu.")
            ...
            print(f"\n📥 QC-Question1: Traitement catégorie '{id_categorie}'.")
            # Créer la requête
            ...
            # Construire le message pour l'étape suivante
            output_message = {
                'id_categorie': id_categorie,
                'is_reset': is_reset,  # Reset déjà fait à l'étape 1
                'step': 2,
                'previous_step': 'question1',
                'status': result.status
            }
            
            # Publier vers l'étape suivante
            self.publisher.publish_message(output_message)
            
            # S'assurer que le channel est ouvert avant l'ACK
            active_ch = self._ensure_channel_open(ch)
            active_ch.basic_ack(delivery_tag=method.delivery_tag)
            print(f"✅ QC-Question1: Catégorie '{id_categorie}' traitée avec succès.")
            
        except (json.JSONDecodeError, ValueError) as e:
            # Erreur de validation: DLQ direct (pas de retry)
            ...
            if self._is_transient_error(e):
                # Erreur transitoire: appliquer la logique de retry
                ...
                if retry_count < MAX_RETRIES:
                    ...
                    print(f"❌ Échec après {MAX_RETRIES + 1} tentatives. Message envoyé à la DLQ. Erreur: {e}")
                    self._send_to_dlq(ch, method, body, e, MAX_RETRIES)
            else:
                # Erreur métier/permanente: DLQ direct (pas de retry)
                self._send_to_dlq(ch, method, body, e, 0)
    ...
    def start_consuming(self):
        ...
        for i in range(3):
            try:
                ...
                print("👂 QC-Question1: En attente de messages...")
