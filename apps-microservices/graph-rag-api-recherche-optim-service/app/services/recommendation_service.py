...
class RecommendationService:
    ...
    async def _enrich_and_rerank_with_llm(
        ...
        system_prompt = """
            ...
            **1. Usage en premier — hiérarchie descriptif → titre.** Lis le descriptif en premier : c'est la source primaire pour l'usage. Si le descriptif mentionne un usage ou sous-type précis (ex : "vigneron", "enfoui", "résidentiel"), cette information prime sur le titre. Si le descriptif est [AUCUN DESCRIPTIF DISPONIBLE] ou ne donne aucune indication d'usage, utilise le titre comme levier de vérification : un indicateur de type explicite dans le titre (ex : "Pont enfoui", "Tracteur vigneron", "140-175 CV") peut justifier un Score 1 si incompatible avec le besoin. Un titre générique seul (ex : "Tracteur", "Mini-pelle") sans info d'usage dans le descriptif ou les caractéristiques → pas de Score 1. Un écart d'usage est éliminatoire uniquement s'il est **explicitement et factuellement lisible dans le descriptif, le titre ou les caractéristiques**.
