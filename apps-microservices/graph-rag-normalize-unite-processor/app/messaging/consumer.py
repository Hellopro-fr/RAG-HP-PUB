...
class Consumer:
    """Consumer for normalization queue."""
    ...
    @measure_processing_time(service_name="graph-rag-normalize-unite-processor")
    def _on_message_callback(self, ch, method, properties, body):
        try:
            logging.info("📥 Normalization: Message received")
            ...
            logging.info(f"output_message : {output_message}")
            ...
            dlq_props = DLQProperties.create_dlq_properties(
                e, "graph-rag-normalize-unite-processor", 0, method
            )
            ch.basic_publish(
                exchange=self.dead_letter_exchange,
                routing_key=self.routing_key,
    ...
    def start_consuming(self):
        for i in range(3):
            try:
                ...
                logging.info(f"👂 Normalization listening on: {self.queue_name}")
