...
class Consumer:
    ...
    def _on_message_callback(self, ch, method, properties, body):
        try:
            ...
            print(f"\n📥 Database-Website-Processor: Message reçu pour URL: {website_data.get('data', [{}])[0].get('url', 'URL inconnue')}")

            # 1. Appelle la logique métier PURE
            output_message = insertion_data(website_data)
        
            # 2. Utilise le publisher pour envoyer le résultat (si ce n'est pas un skip)
            if output_message.get("status") != "skipped":
