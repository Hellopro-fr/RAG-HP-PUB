<template>
  <div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 px-4 py-3 gap-3 flex-wrap">
      ...
      <div class="p-4 sm:p-6">
        ...
        <div v-else-if="!filteredTables.length" class="text-center py-12 text-gray-500 dark:text-gray-400">
...
const activeDb = ref<number>(parseInitialDb());
...
function parseInitialDb(): number {
  const raw = Number(route.query.db);
  if (databases.some((d) => d.id === raw)) return raw;
...
async function loadTables() {
  loading.value = true;
  try {
    const res = await bddApi.listUsed(activeDb.value);
...
function setActiveDb(id: number) {
  activeDb.value = id;
  router.replace({ query: { ...route.query, db: String(id) } });
  // Collapse all expanded rows when switching DB
...
async function confirmDeleteTable() {
  ...
  const id = deletingTable.value.id;
  try {
    await bddApi.deleteUsed(id);
...
function handleCreated(created: BDDUsedTable) {
  ...
  if (created.database_id === activeDb.value) {
    ...
    // user picked another DB inside the dialog — switch tab to show it
    activeDb.value = created.database_id;
  }
}
</script>
