  ...
  <div class="fixed inset-0 bg-black/50 z-40 flex items-center justify-center" @click.self="emit('close')">
    <div class="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-900">Détails du serveur</h2>
        <button class="p-1.5 rounded hover:bg-gray-100 text-gray-500" @click="emit('close')">
          <i class="pi pi-times" />
        </button>
      </div>
      ...
      <template v-else-if="detail">
        ...
        <div
          ...
          class="bg-red-50 border border-red-200 rounded-md p-3 mb-4 text-sm text-red-700"
        >
          <i class="pi pi-exclamation-triangle mr-1" />
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
          <div>
            ...
            <p class="text-sm font-mono text-gray-800 truncate">{{ detail.id }}</p>
          </div>
          <div>
          ...
          <div v-if="detail.url">
            ...
            <p class="text-sm text-gray-800 truncate">{{ detail.url }}</p>
          </div>
          <div v-if="detail.message_url">
            <span class="text-xs text-gray-500">Message URL</span>
            <p class="text-sm text-gray-800 truncate">{{ detail.message_url }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Transport</span>
            <p class="text-sm text-gray-800">{{ detail.mcp_transport }} ({{ detail.transport_preference }})</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Serveur</span>
            <p class="text-sm text-gray-800">
              {{ detail.server_name || '—' }}
              <span v-if="detail.server_version" class="text-gray-500">v{{ detail.server_version }}</span>
            </p>
          </div>
        </div>
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between mb-2">
            <h3 class="text-sm font-semibold text-gray-900">Configuration MCP JSON</h3>
            <button
          ...
          <pre class="bg-gray-50 border border-gray-200 rounded-md p-3 text-xs font-mono overflow-x-auto max-h-48">{{ mcpJsonString }}</pre>
        </div>

        <!-- Tools -->
        <div class="mb-6">
          <div class="flex items-center gap-2 mb-3">
            <h3 class="text-sm font-semibold text-gray-900">Outils</h3>
            <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
              {{ activeToolsCount }}/{{ detail.tools.length }} actifs
            </span>
          </div>
          <div v-if="detail.tools.length === 0" class="text-sm text-gray-500">Aucun outil découvert</div>
          <div v-else class="space-y-2 max-h-64 overflow-y-auto">
            <div
              v-for="tool in detail.tools"
              :key="tool.name"
              class="flex items-center justify-between p-2 rounded border border-gray-100 bg-gray-50"
            >
              <div class="min-w-0 flex-1">
                ...
                <p v-if="tool.description" class="text-xs text-gray-500 truncate mt-0.5">{{ tool.description }}</p>
              </div>
              <button
                class="ml-2 p-1 rounded hover:bg-gray-200 text-gray-500 text-xs"
        ...
        <div class="mb-6">
          ...
          <div v-if="detail.resources.length === 0" class="text-sm text-gray-500">Aucune ressource</div>
          <div v-else class="space-y-2 max-h-48 overflow-y-auto">
            <div
              v-for="resource in detail.resources"
              :key="resource.uri"
              class="p-2 rounded border border-gray-100 bg-gray-50"
            >
              <span class="text-sm font-medium text-gray-800">{{ resource.name }}</span>
              <p class="text-xs font-mono text-gray-500 truncate">{{ resource.uri }}</p>
              <p v-if="resource.description" class="text-xs text-gray-500 mt-0.5">{{ resource.description }}</p>
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="flex items-center gap-2 mb-3">
            <h3 class="text-sm font-semibold text-gray-900">Prompts</h3>
            <span class="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
              {{ detail.prompts.length }}
            </span>
          </div>
          <div v-if="detail.prompts.length === 0" class="text-sm text-gray-500">Aucun prompt</div>
          <div v-else class="space-y-2 max-h-48 overflow-y-auto">
            <div
              ...
              class="p-2 rounded border border-gray-100 bg-gray-50"
            >
              <span class="text-sm font-medium text-gray-800">{{ prompt.name }}</span>
              <p v-if="prompt.description" class="text-xs text-gray-500 mt-0.5">{{ prompt.description }}</p>
              <div v-if="prompt.arguments?.length" class="mt-1">
                <span class="text-xs text-gray-400">Arguments :</span>
                <ul class="list-disc list-inside ml-1">
                  ...
                  >
                    ...
                    <span v-if="arg.description" class="text-gray-400"> — {{ arg.description }}</span>
                  </li>
                </ul>
              </div>
            </div>
